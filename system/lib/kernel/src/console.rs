...
impl<'g> Console<'g> {
    ...
    /// Renders raw `buf` bytes through the same escape-parsing path as
    /// [`print`](Self::print) — the byte-oriented twin for the floor's `write`
    /// fan-out, which carries `&[u8]` rather than `&str`. Every fd 1/2 write
    /// reaches the on-screen console here, so it renders with the same SGR
    /// coloring as a `print` of the same bytes.
    ...
    /// [`Action`]s (printable cells, cursor moves, SGR pen changes).
...
/// UART-only.
struct ConsoleSink(UnsafeCell<Option<Console<'static>>>);
...
// L12 layout: tests live in the sibling file `console_tests.rs`, declared as a
// `#[path]` child so `super::` reaches the private blit/blend items. The file
