...
- **depgraph**: Retired the `rendersurface_migration_progress` probe (purpose fulfilled by trait rename). Renamed `cellcapability_implements_rendersurface` → `cellcapability_implements_chromesurface` and updated the grep pattern to match the renamed impl block. Added new tripwire probe `no_render_surface_legacy_name` that scans workspace `*.rs` sources (excluding `archive/` and `target/`) for any re-introduction of the legacy `trait RenderSurface` / `dyn RenderSurface` / `use …::RenderSurface` name shapes. TypeScript files under `clients/web/**` are deliberately excluded — the web client owns its own `RenderSurface` TS rename as part of a later flight.
- **codec**: Established client-side codec foundation (#753). New crate `reovim-client-subsys-codec` hosts `FrameTarget` (typeid-keyed capability slots), `RenderHandler`/`RenderHandlerRegistry`, and `SurfaceEncoder`/`SurfaceEncoderRegistry`. New crate `reovim-ext-client-tui-cap-cell` defines the TUI cell-grid capability type. Retired seven unused server-side crates that had zero production consumers: `reovim-subsys-render-codec`, `reovim-subsys-surface-codec`, `reovim-render-codec`, `reovim-surface-codec`, `reovim-render-codec-tui`, `reovim-surface-codec-tui`, `reovim-surface-codec-pixel`. New depgraph test `codec_foundation_layout.rs` locks the invariant (foundation present + retired tree absent + client subsys kernel-free). TUI render pipeline cutover lands in a follow-on flight.
- **codec**: TUI surface-descriptor dispatch now routes through `SurfaceDescriptorHandler` registry (#753). New trait + `DefaultSurfaceDescriptorHandlerRegistry` added to `reovim-client-subsys-codec`. New ext crate `reovim-tui-mod-surface-descriptor-cell-grid` implements the first built-in handler (kind=`0x0001` → `CellGridSurfaceInfo { width, height }`). `clients/tui/src/notification_handler.rs` hardcoded `desc.kind == 0x0001` pattern removed; dispatch now goes through `surface_decoding::global_registry()` populated at `TuiApp::new` startup. Two new depgraph probes (`tui_surface_handler_registered`, `no_hardcoded_surface_decode_leaks`) lock the invariant. `RenderSurface` trait deletion (57-file ripple) deferred to a follow-on flight.
- **codec**: Added narrow `SurfaceApplyContext` trait + `SurfaceDescriptorApplyError` enum + `SurfaceDescriptorHandler::decode_and_apply` method to `reovim-client-subsys-codec` (#753). The routing layer in `clients/tui/src/notification_handler.rs` now calls `handler.decode_and_apply(&desc.body, ctx)` — it no longer imports concrete descriptor types from ext handler crates and never sees `Box<dyn Any + Send>`. `CellGridSurfaceHandler` overrides `decode_and_apply` to call `ctx.set_surface_size(w, h)` directly; zero-dimension descriptors surface `StateApplyRejected` instead of silently no-oping. `decode_surface_descriptor` function length dropped ~70→~20 lines. Two new depgraph probes (`surface_apply_context_exists`, `no_concrete_descriptor_import_in_notification_handler`) enforce the symbol presence and the absence of concrete-type imports.
- **codec**: Plan 23 / 17-β.2b-impl-c — bulk consumer migration across 8 TUI chrome modules (10 test files) (#753). Applied the Plan 22 recipe to `which-key`, `tetromino` (2 files), `signature-help`, `notification`, `microscope` (2 files), `hover`, `completion`, and `cmdline`: replaced `RecordingSurface` test fixtures with `CellCapability::new`, added per-file `has_content` / `char_at` / `style_at` / `text_at_row` helpers as needed. Color-assertion rewrites go via the 19-arm `CellColor` palette (Red=Named(9), Blue=Named(12), Yellow=Named(11), etc.). `statusline` deferred to 17-β.2c (its tests depend on `RecordingSurface::writes()` call-log semantics that `CellCapability` can't replicate). Runtime signatures still use `&mut dyn RenderSurface` — trait deletion stays for 17-β.2c.
- **codec**: Plan 22 / 17-β.2b-impl-b — pilot consumer migration: the `landing` TUI chrome module's tests now construct a `CellCapability` directly instead of using the driver-side `RecordingSurface` fixture (#753). Runtime signatures still use `&mut dyn RenderSurface`; test-fixture replacement is the entire migration shape (trait deletion stays for 17-β.2c). Also folds two prior non-blockers: the Plan 21 zero-dim rect fill test (proves `RenderSurface::fill` is a noop on width-zero or height-zero rects) and the Plan 20 `scan_file` brace-counter off-by-one fix in the decode-caller probe (moved arm-check before char walk; added a nested-scope self-test to lock the behaviour). 14 remaining chrome modules will migrate in 17-β.2b-impl-c following the same pattern.
- **codec**: Plan 21 / 17-β.2b-impl-a — `impl RenderSurface for CellCapability` landed (#753). The ext crate `reovim-ext-client-tui-cap-cell` now implements the `RenderSurface` trait for `CellCapability` via a `Style` → `CellStyle` translation layer (19-arm exhaustive `Color` match, per-flag attribute translation; `STRIKETHROUGH` is explicitly dropped since `CellAttrs` has no counterpart). `ExtClient{tui}` → `RepoCore` edge (allowed by the classifier). This is the architectural unlock for 17-β.2b-impl-b: every existing `&mut dyn RenderSurface` call-site now accepts a `CellCapability` without call-site changes. New depgraph probe (`cellcapability_implements_rendersurface`) locks the impl + dep edge. 28 new unit tests; 61 total in the cell crate.
- **codec**: Plan 20 / 17-β.2b-pre — precondition refactors ahead of `RenderSurface` consumer migration (#753). Extracted private `decode_info` helper in `reovim-tui-mod-surface-descriptor-cell-grid`, removing the `.expect` downcast panic site that FD flagged in the Flight 66 landing review. Added two new depgraph tripwires: `no_pub_decode_caller_outside_tests` (restricts production callers of `SurfaceDescriptorHandler::decode()` — the boxed pathway must stay a test-only affordance; probe includes a self-test seeding a forbidden pattern to lock detection behaviour) and `rendersurface_migration_progress` (baseline range-tripwire for `RenderSurface` file references; tightens as 17-β.2b-impl and 17-β.2c land). **Consumer migration deferred**: the original 17-β.2b scope was split mid-flight — `impl RenderSurface for CellCapability` and the ~30 consumer-file migrations move to a follow-on 17-β.2b-impl flight; see plan 20 §9 for the scope-reduction rationale.
...
- **Plan 16 Phase G — depgraph guard + docs sweep** (#753): the post-migration invariant is now machine-enforced. New `lib/depgraph/tests/capabilities_decentralized.rs` with 3 tests:
  - `no_reovim_capabilities_workspace_member` (via `cargo_metadata`)
  - `no_reovim_capabilities_dep_edge` (walks each workspace package's `Cargo.toml` dep tables)
  - `no_reovim_capabilities_rust_reference` (workspace source walk with comment-strip; exclusions: `archive/`, `*.md`, `*.proto`, self-file)
  - Each guard verified via probe-break (re-introduce a violation, confirm the test panics with the expected message, revert). Pattern mirrors `lib/depgraph/tests/proto_no_v2_alias.rs` (Plan 15 Phase D). Docs sweep: `.claude/rules/architecture.md`, `docs/architecture/overview.md`, `docs/contributing/getting-started.md` — removed all `lib/capabilities/` / `reovim-capabilities` references.
