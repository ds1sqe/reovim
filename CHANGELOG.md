...
- **BREAKING (internal)**: `reovim-domain-text`, `reovim-domain-text-events`, and `reovim-provider-text` removed from `reovim-server` `[dependencies]`. Server is now a domain-agnostic dispatch layer. All text-domain types accessed via driver re-exports (`reovim-driver-session`, `reovim-driver-buffer`). `insert_char_for_client` deleted, `FallbackContext` impl removed from `AppState`, gRPC serialization decoupled (`yank_type_str()`, `line_to_byte_offset()`, `text_edit_to_decoded_edit` moved to driver-codec), syntax event conversion moved to driver-syntax. Depgraph guard test `server_no_domain_text` permanently prevents regression
