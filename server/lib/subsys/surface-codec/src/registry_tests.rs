//! Tests for `SurfaceCodecRegistry` — register/unregister/get contract.

use {
    super::registry::{DefaultSurfaceCodecRegistry, SurfaceCodecRegistry},
    reovim_surface_codec::{Codec, SurfacePayloadError},
...
fn get_on_empty_registry_returns_none() {
    let reg = DefaultSurfaceCodecRegistry::new();
...
fn default_registry_is_empty() {
    let reg = DefaultSurfaceCodecRegistry::default();
    assert!(reg.get(0x0001).is_none());
}
...
fn default_registry_debug_lists_registered_kinds() {
    ...
    let dbg = format!("{reg:?}");
    assert!(dbg.contains("DefaultSurfaceCodecRegistry"));
    assert!(dbg.contains("66") || dbg.contains("0x42") || dbg.contains("42"));
}
...
fn concurrent_register_and_get() {
    ...
    let reg = Arc::new(DefaultSurfaceCodecRegistry::new());
...
fn arc_dyn_trait_object_works() {
    let reg: Arc<dyn SurfaceCodecRegistry> = Arc::new(DefaultSurfaceCodecRegistry::new());
