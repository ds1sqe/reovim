...
///
/// ```
/// use reovim_subsys_command_types::{CommandContext, ArgValue};
///
...
impl CommandContext {
    ...
    ///
    /// Returns `(line, column)` as zero-based indices.
    ///
    /// # Example
    ///
    /// ```ignore
    /// fn execute(&self, runtime: &mut SessionRuntime<'_>, args: &CommandContext) -> CommandResult {
    ///     let Some((line, col)) = args.cursor_position() else {
    ///         return CommandResult::error("No cursor position");
    ///     };
    ///     // Use (line, col) for operation...
    /// }
    /// ```
    #[must_use]
    ...
    pub fn set_cursor_position(&mut self, line: usize, col: usize) {
        self.set("cursor", ArgValue::Position(line, col));
    }
}
