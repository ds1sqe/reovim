...
fn test_command_context_set_cursor_position_origin() {
    ...
    let (line, col) = ctx.cursor_position().expect("cursor should be set");
    assert_eq!(line, 0);
    assert_eq!(col, 0);
}
...
fn test_command_context_multiple_args() {
    ...
    let (line, col) = ctx.cursor_position().unwrap();
    assert_eq!(line, 5);
    assert_eq!(col, 2);
}
