...
fn new_window_has_initial_cursor() {
    ...
    let win = Window::new(WindowId::from_raw(1), BufferId::from_raw(10), 1, cursor);
...
fn window_accessors() {
    let cursor = Box::new(TestCursor::new(1, 0));
    let win = Window::new(WindowId::from_raw(5), BufferId::from_raw(20), 1, cursor);
...
fn set_cursors_replaces_all() {
    let cursor = Box::new(TestCursor::new(1, 0));
    let mut win = Window::new(WindowId::from_raw(1), BufferId::from_raw(10), 1, cursor);
    assert_eq!(win.cursors().len(), 1);
