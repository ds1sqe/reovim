...
use {
    reovim_protocol::v2::{
        ...
        GetModeResponse, GetOptionsRequest, GetOptionsResponse, GetScreenContentRequest,
        GetScreenContentResponse, GetSelectionRequest, GetSelectionResponse,
        GetVisibleLinesRequest, GetVisibleLinesResponse, Position,
...
impl StateService for StateServiceImpl {
    ...
    /// Get screen content.
    ///
    /// Not yet implemented - requires render infrastructure.
    /// TUI clients should use client-side rendering with buffer/layout data.
    async fn get_screen_content(
        &self,
        _request: Request<GetScreenContentRequest>,
    ) -> Result<Response<GetScreenContentResponse>, Status> {
        Err(Status::unimplemented(
            "GetScreenContent not yet implemented - use client-side rendering",
        ))
    }
}
