...
mod snapshot_tests {
    ...
    // === VirtualBuffer Snapshot Tests ===

    fn make_vbuf(content: &str) -> crate::mm::VirtualBuffer {
        use std::sync::Arc;
        let original: Arc<dyn crate::mm::FileMapping> =
            Arc::new(crate::mm::HeapMapping(content.as_bytes().to_vec()));
        let line_index = crate::mm::LineIndex::build_from_str(content);
        crate::mm::VirtualBuffer::new(original, line_index)
    }

    #[test]
    fn virtual_capture_and_restore() {
        let mut vbuf = make_vbuf("Hello\nWorld");
        let cursor = Position::new(1, 3);
    ...
    #[test]
    fn rope_snapshot_not_virtual() {
        let buffer = Buffer::from_string("rope");
        let snap = Snapshot::capture(&buffer, Position::origin());
        assert!(!snap.is_virtual());
        let debug = format!("{snap:?}");
