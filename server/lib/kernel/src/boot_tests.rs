...
//! - `Init::boot` returns a `Kernel` with a live `event_bus` after success.
//! - `Init::boot` with an injected failure returns `Err(BootError::Stage)`.
...
// ── Full-boot integration smoke via Init::boot ───────────────────────────────
//
// Verifies that Init::boot succeeds and returns a Kernel with a live
// event_bus Shared. The bus + stage driver are already smoke-tested via
// run_boot_stages above; here we verify the Init::boot wrapper does not
// break the invariant.

arch_test!(init_boot_returns_kernel_with_event_bus, {
    ...
    assert_eq!(
        ...
        1,
        "Kernel.event_bus strong count must be 1 immediately after boot"
    );
});
...
// ── Forced-failure via Init::boot drops Init whole (LF13) ──────────────────
