...
fn test_set_screen_no_compositor() {
    use {crate::testing::TestSessionRuntime, reovim_subsys_layout::Rect};
...
fn test_show_overlay_no_compositor() {
    use {crate::testing::TestSessionRuntime, reovim_subsys_layout::OverlayConstraints};
...
fn test_to_kernel_split_direction() {
    use reovim_subsys_layout::SplitDirection;
...
#[cfg_attr(coverage_nightly, coverage(off))]
impl reovim_subsys_clipboard::ClipboardProvider for MockClipboard {
...
struct MockUndoProvider {
    edits: std::sync::Mutex<Vec<reovim_driver_text_undo::UndoRecord>>,
}
...
impl reovim_driver_text_undo::UndoProvider for MockUndoProvider {
    ...
    fn record(
        ...
        self.edits
            .lock()
            .unwrap()
            .push(reovim_driver_text_undo::UndoRecord {
                buffer_id,
                edits,
                cursor_before,
                cursor_after,
...
#[cfg_attr(coverage_nightly, coverage(off))]
impl reovim_driver_text_undo::UndoProvider for AlternateUndoProvider {
    fn undo(&self, _buffer_id: BufferId) -> Option<UndoResult> {
...
struct MockLayerCompositor {
    id: reovim_subsys_layout::LayerId,
...
impl MockLayerCompositor {
    fn new() -> Self {
        ...
        Self {
            id: reovim_subsys_layout::LayerId::new(0),
...
impl reovim_subsys_layout::WindowLayerCompositor for MockLayerCompositor {
    ...
    fn arrange(&self, _bounds: Rect) -> Vec<reovim_subsys_layout::WindowPlacement> {
        Vec::new()
    }
    ...
    fn show_overlay(&mut self, _constraints: reovim_subsys_layout::OverlayConstraints) -> WindowId {
        let id = WindowId::from_raw(self.next_id);
        self.next_id += 1;
        id
    }
    ...
    fn windows_in_zone(&self, zone: reovim_subsys_layout::Zone) -> Vec<WindowId> {
        if zone == reovim_subsys_layout::Zone::Tiled {
            self.windows.clone()
        } else {
            Vec::new()
        }
    ...
    fn zone_of(&self, _window: WindowId) -> Option<reovim_subsys_layout::Zone> {
        Some(reovim_subsys_layout::Zone::Tiled)
    }
}
...
impl reovim_subsys_layout::RootCompositor for MockRootCompositor {
    ...
    fn layer_of(&self, _window: WindowId) -> Option<reovim_subsys_layout::LayerId> {
        Some(reovim_subsys_layout::LayerId::new(0))
    }
...
fn test_compositor_split() {
    ...
    let result = rt.split(reovim_subsys_layout::SplitDirection::Horizontal);
    assert!(result.is_ok());
...
fn test_compositor_show_overlay() {
    ...
    let result =
        rt.show_overlay(reovim_subsys_layout::OverlayConstraints::centered().with_size(20, 10));
    assert!(result.is_ok());
}
...
#[cfg_attr(coverage_nightly, coverage(off))]
impl reovim_subsys_layout::WindowLayerCompositor for SingleWindowLayerCompositor {
    fn id(&self) -> reovim_subsys_layout::LayerId {
        self.id
    }
...
#[cfg_attr(coverage_nightly, coverage(off))]
impl reovim_subsys_layout::RootCompositor for SingleWindowRootCompositor {
    fn composite(&self, screen: Rect) -> reovim_subsys_layout::CompositeResult {
        reovim_subsys_layout::CompositeResult::empty(screen)
    }
    fn create_layer(
...
fn test_compositor_close_current_window_single_window() {
    ...
    let mut w = crate::WindowLayout::empty();
    let mut e = crate::ExtensionMap::new();
    let mut c: Option<Box<dyn reovim_subsys_layout::RootCompositor>> =
...
fn test_set_screen_without_compositor() {
    ...
    let mut e = crate::ExtensionMap::new();
    // No compositor
    let mut c: Option<Box<dyn reovim_subsys_layout::RootCompositor>> = None;
    let mut tabs = crate::TabPageSet::new();
    let mut r = RegisterBank::new();
    let mut ch = HistoryRing::new();
    let mut lm = MarkBank::new();
