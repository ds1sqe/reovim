...
use {
    reovim_driver_text_ffi::with_runtime,
    reovim_driver_text_session::api::{
...
impl PyRuntimeApi {
    ...
    fn push_mode(&self, mode_id: &str) -> PyResult<()> {
        ...
        with_runtime(|rt| {
            rt.push_mode(mode, reovim_driver_text_session::TransitionContext::new());
        })
        .map_err(|_| PyRuntimeError::new_err("no active runtime"))
    }
    ...
    fn set_mode(&self, mode_id: &str) -> PyResult<()> {
        ...
        with_runtime(|rt| {
            rt.set_mode(mode, reovim_driver_text_session::TransitionContext::new());
        })
        .map_err(|_| PyRuntimeError::new_err("no active runtime"))
    }
