...
use {
    ...
    reovim_content_codec::{
        ByteNotifiable, CodecError, CodecMetadata, ContentType, DecodeResult, DecodedEdit,
        DomainEdit, MountMode, TranslateEditError,
    },
...
fn apply_decoded_edit_updates_source() {
    ...
    let byte_edit = state
        .apply_decoded_edit(
            buf(1),
            "default",
            &DecodedEdit::Domain(DomainEdit::new(TextEdit {
...
impl ByteNotifiable for MockIndex {
...
use reovim_content_codec::{CodecView, ContentCodec, ContentCodecFactory, SwitchViewError};
...
fn multi_view_store() -> reovim_content_codec::ContentCodecFactoryStore {
    let store = reovim_content_codec::ContentCodecFactoryStore::new();
...
use reovim_content_codec::{MountCodecError, UmountCodecError};
...
fn unmount_codec_not_found_returns_error() {
    ...
    let bogus = reovim_content_codec::MountId::from_u64(999_999);
...
fn unmount_codec_skips_buffers_without_inode() {
    ...
    state.unmount_codec(handle.mount_id()).unwrap();
...
fn unmount_codec_all_buffers_without_inode() {
    ...
    let bogus = reovim_content_codec::MountId::from_u64(999);
    let err = state.unmount_codec(bogus).unwrap_err();
    assert_eq!(err, UmountCodecError::MountNotFound);
}
