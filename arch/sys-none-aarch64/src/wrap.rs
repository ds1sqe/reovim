...
//! binds to the machine instead of a kernel: byte I/O is the PL011 UART,
//! the clock is the generic timer, pages come from the static arena, and
//! exit is the semihosting channel. Services with no hardware realization
//! (files, threads, page protection) fail through the floor's existing
//! error vocabulary instead of growing a capability API.
//!
...
/// `CLOCK_MONOTONIC` — the generic-timer counter.
...
/// Reads bytes from UART0 for fd 0.
///
...
/// Reads the generic timer into `tp`.
///
/// `CLOCK_MONOTONIC` is the counter divided by its firmware-programmed
/// frequency. `CLOCK_REALTIME` returns the same boot-relative value: the
/// machine has no RTC, so a fabricated wall-clock epoch would be a lie —
/// callers get a clock that is honest about being boot-relative.
///
/// # Errors
///
/// `EINVAL` for any other clock id, or if the firmware left the counter
/// frequency unprogrammed (zero).
...
/// Futex wait/wake over the event stream: `wfe` to sleep, `sev` to wake.
///
...
/// (wait forever), and there is no kernel timer to arm. `FUTEX_WAKE`
/// broadcasts (`sev` wakes every `wfe` core) and reports 0 woken — all
/// callers discard the count. These wait/wake paths are reachable only
/// through spawned threads, which this target's `clone_into` cannot create;
/// the dumbest correct form is deliberate.
///
/// # Errors
///
...
pub fn futex(
    ...
    match op & !FUTEX_PRIVATE_FLAG {
        ...
        FUTEX_WAKE => {
            // SAFETY: `sev` only signals the event register on every core.
            unsafe {
...
/// Terminates the run with `code` via the semihosting channel.
///
