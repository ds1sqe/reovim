//! Tests for `sys/net.rs`, compiled into the lib under `selftest` (#797 Phase 1).
//!
//! L12 layout: sibling file declared in `sys/mod.rs` via
//! `#[cfg(feature = "selftest")] mod net_tests;`. Tests access only public
//! items from `crate::sys::net`, so `super::` crate-private access is not needed.

use crate::{
    arch_test,
    sys::net::{AF_UNIX, SOCK_STREAM, SockaddrUn, UNIX_PATH_MAX},
    testrt,
};
