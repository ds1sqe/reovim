...
///
/// ```rust
/// use reovim_arch::sys::net::{SockaddrUn, AF_UNIX, UNIX_PATH_MAX};
///
...
impl SockaddrUn {
    ...
    ///
    /// ```rust
    /// use reovim_arch::sys::net::SockaddrUn;
    ///
    ...
    ///
    /// ```rust
    /// use reovim_arch::sys::net::{SockaddrUn, AF_UNIX};
    ///
    /// let mut sa = SockaddrUn::zeroed();
    /// sa.sun_family = AF_UNIX;
...
// L12: tests live in sibling file `net_tests.rs`, declared in `sys/mod.rs`
// under `#[cfg(feature = "selftest")]`. Public items have doc-tests above.
