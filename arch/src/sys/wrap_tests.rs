//! Tests for `sys/wrap.rs`, compiled into the lib under `selftest` (#785 Phase 5).
//!
//! L12 layout: sibling file declared in `sys/mod.rs` via
//! `#[cfg(feature = "selftest")] mod wrap_tests;`. Tests access only public
//! items from `crate::sys`, so `super::` crate-private access is not needed.
