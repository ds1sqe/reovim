//! Tests for `sys/errno.rs`, compiled into the lib under `selftest` (#785 Phase 5).
//!
//! L12 layout: sibling file declared in `sys/mod.rs` via
//! `#[cfg(feature = "selftest")] mod errno_tests;`. The tests access only
//! public items (`Errno`, `from_ret`, named constants), so crate-private
//! access through `super::` is not needed.

use crate::{
    arch_test,
    sys::{EAGAIN, EBADF, ENOMEM, EWOULDBLOCK, Errno, from_ret},
    testrt,
};
