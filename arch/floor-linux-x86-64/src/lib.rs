//! `reovim-arch-floor-linux-x86-64` — the `x86_64`-linux language floor.
//!
//! This `#![no_std]` crate owns the per-binary **language-knowledge** symbols
//! for `x86_64-linux` (OS-Modes §2.2, master-plan invariant #1 — the LINK, not
...
//!
//! It reaches the OS only through its matching `reovim-arch-sys-linux-x86-64`
...
// `warnings = deny`.
#[cfg(feature = "runtime")]
use reovim_arch_sys_linux_x86_64 as sys;
...
/// #![no_std]
/// #![no_main]
/// reovim_arch_floor_linux_x86_64::entry!(|_argc, _argv, _envp| {
///     // ... boot, do work ...
///     0
/// });
/// ```
