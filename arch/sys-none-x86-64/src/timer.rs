...
/// idiom.
// Crate-visible (boot_info reads CPUID leaves through it); `timer` is a private
// module, so clippy's nursery `redundant_pub_crate` reads the crate-scope as
// redundant — but the visibility is real (cross-module within the crate), and
// widening to `pub` would falsely mark a private-module helper as public API.
...
// L12 layout: tests live in the sibling file `timer_tests.rs`, declared as a
// `#[path]` child so `super::` reaches the private frequency-derivation helper
// and the fallback constant. The file is only compiled when this module is (the
// lib.rs target gate), so the inner declaration needs only the `selftest`
// feature gate.
