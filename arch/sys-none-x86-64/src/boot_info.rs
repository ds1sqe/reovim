...
//! which `_start` stashes into [`crate::MULTIBOOT_INFO_PTR`]. This module
...
///
/// The storage cell lives in [`crate::MULTIBOOT_INFO_PTR`] (relocated into this
/// crate by the SP01 edge inversion); this is the single read API.
#[must_use]
fn multiboot_ptr() -> u32 {
    crate::MULTIBOOT_INFO_PTR.load(Ordering::Relaxed)
}
...
// a `#[path]` child so `super::` reaches the private mmap parser, the type
// classifier, and the pointer accessor. The file is only compiled when this
// module is (the lib.rs target gate), so the inner declaration needs only the
// `selftest` feature gate.
