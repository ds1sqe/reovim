//! Tests for x86_64 Multiboot/E820 boot-memory discovery.
...
arch_test!(boot_info_parse_counts_without_writing, {
    // Empty `out`: the sizing path counts every entry, writes none.
    let mut buf = [0u8; 48];
...
arch_test!(boot_info_parse_zero_size_halts, {
    ...
    let mut buf = [0u8; 48];
    write_entry(&mut buf[0..24], 20, 0x0, 0x1000, 1);
    write_entry(&mut buf[24..48], 0, 0x2000, 0x1000, 1);
    let mut out = [blank(); 4];
    // SAFETY: readable 48-byte blob owned by this frame.
    let n = unsafe { parse_entries(buf.as_ptr(), buf.len(), &mut out) };
...
arch_test!(boot_info_mmap_region_reads_header, {
    ...
    let mut hdr = [0u8; 52];
    hdr[0..4].copy_from_slice(&FLAG_MMAP.to_le_bytes());
    hdr[44..48].copy_from_slice(&48u32.to_le_bytes());
    hdr[48..52].copy_from_slice(&0x1000u32.to_le_bytes());
