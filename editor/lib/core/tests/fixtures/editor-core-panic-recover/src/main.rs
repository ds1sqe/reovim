//! AB12 editor-core recover-path fixture (#796 Phase 5).
//!
...
entry!(|argc, argv, _envp| {
    ...
    // Boot the editor core with `Recover` disposition (75 exit code on panic).
    // `boot()` registers:
    ...
    panic!("recover-path editor-core fixture panic");
});
