//! AB12 editor-core halt-path fixture (#796 Phase 5).
//!
//! Boots via arch `_start`, opens the LOG7 sink file named by `argv[1]`,
//! registers it as the panic flush fd, boots the editor core (which registers the
//! remaining three arch panic-seam hooks: ring_tail provider, state-record
//! hook, and halt disposition), emits a few events to populate the ring/flush
//! buffer, then panics. The panic handler flushes the ring's LOG2 lines
//! followed by the panic line to the sink file and exits the halt code (70).
//!
//! The integration test (`tests/fixtures_exec.rs`) execs this fixture, asserts
//! exit code 70, and parses the flushed file content for LOG2 grammar
//! conformance and the presence of the panic line.
//!
//! State-record AC (c) — "the state-record stub observed the matching
//! PanicRecord" — cannot be asserted out-of-process because the editor core's
...
entry!(|argc, argv, _envp| {
    ...
    // `EditorInit::boot()` reads it. The fixture is host-only (native-only pin per
    ...
    // SAFETY: `argc`/`argv` are the floor-supplied startup vectors `_start`
    ...
    // Boot the editor core. `LauncherArgs` default uses `Recover` disposition; for
    // the halt fixture we override to `Halt`. `boot()` registers:
    //   1. ring_tail_provider  → flush::ring_tail
    //   2. state_record_hook   → record_panic_state (AtomicU32 slot)
    //   3. disposition         → Halt (exit 70)
    ...
    let _editor_core = match EditorInit::new(args).boot() {
        Ok(k) => k,
        Err(_) => return 3,
    };
    ...
    //      (the boot stages emitted boot.stage.* events which populated
    //      the flush mirror; the harness parses these for LOG2 conformance)
    //   2. panic line rendered + flushed to fd
    //   3. state_record_hook(PanicRecord{Halt, rollback_failed:false}) called
    //   4. exit(70) — Disposition::Halt exit code
    //
    // This return is never reached.
    panic!("halt-path editor-core fixture panic");
});
...
/// # Safety
///
/// `argv` must be the floor-supplied startup vector: `argc` C-string entries
...
unsafe fn open_sink_from_argv(argc: usize, argv: *const *const u8) -> Option<i32> {
    ...
    // SAFETY: `path_ptr` is a floor-supplied NUL-terminated argv string.
