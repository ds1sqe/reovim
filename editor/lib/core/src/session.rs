...
//!
//! The existing `EditorCore` `()` placeholders (`config`, `lockfile`, `inventory`,
//! `correlation_alloc`, `force_overrides`) are NOT refilled here. The skeleton
//! grows NEW fields (`session`, `domain_router`) onto `EditorCore`.
...
// editor-core/ext boundaries. Re-exported here so callers that name
// `reovim_editor_core::session::*` continue to compile.
...
/// Bounded focus-chain depth for the Phase 4 editor-core substrate.
...
/// EditorCore-owned table of live sessions for Phase 3 fixtures.
///
/// The legacy [`EditorCore::session`](crate::root::EditorCore::session) field remains
...
/// A single editing session (walking-skeleton: one session per editor core).
///
...
impl Session {
    ...
    ///
    /// This is the CC14-safe path used by `EditorCore`: router/session locks are
