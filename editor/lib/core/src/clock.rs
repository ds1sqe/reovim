...
//! the instant `EditorInit::new` is called (boot stage 0). All LOG2 timestamps are
...
///
/// Created once in `EditorInit::new` (boot stage 0) and transferred into `EditorCore`
...
///
/// - `mono_zero`: the monotonic reading at `EditorInit::new`. Subtract it from a
...
impl BootClock {
    ...
    /// anchor.
    ///
    /// Called once in `EditorInit::new`. The two clocks are read back-to-back with
