...
//! boot: no later stage starts, no later events fire, `EditorInit` drops whole and
//! no `EditorCore` is constructed (LF13).
//!
...
//! a deferred `EditorCore` field. OBS2's exemption covers all boot-stage events
...
///
/// ```rust,ignore
/// // ignore: requires the selftest feature — run via the editor-core-selftest bin.
/// use reovim_editor_core::boot::inject_stage_failure;
...
///
/// Stage 0 has already run in `EditorInit::new` (`BootClock` capture); this function
/// drives stages 1..6 then hands off at stage 7 by returning `Ok(())`.
/// The caller (`EditorInit::boot`) wraps the return with the actual `EditorCore`
/// construction.
///
...
/// use reovim_editor_core::boot::run_boot_stages;
///
...
pub fn run_boot_stages(bus: &DS12EventBus, clock: &BootClock) -> Result<(), BootError> {
    ...
    // ── Stage 7: handoff ──────────────────────────────────────────────────
    // Stage 7 marks the transition from EditorInit to EditorCore (EditorInit::boot
    // returns Shared<EditorCore>). Emitting start/ok here signals that the
    // boot sequence has completed and serving begins. The bus is transferred
    // into EditorCore by the caller immediately after this function returns.
