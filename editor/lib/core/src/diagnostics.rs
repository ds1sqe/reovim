...
//!
//! Run at the boot tail (after the stage-7 handoff, before `EditorInit::boot`
...
/// [`run_at_boot_tail`] on the single-threaded `EditorInit::boot` path. The buffer is
...
///
/// Called once from `EditorInit::boot` after the stages run, single-threaded. Each
