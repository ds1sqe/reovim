...
//!
//! The contract row shapes live in `reovim-subsys-domain` (DAG2). The editor core
//! owns only storage, registration mechanics, and dispatch lookup.
//!
//! ## Core/ext boundary
//!
//! `DomainRouter` is an editor-core type. It does NOT depend on
//! `ext/server/domain/*`. The composition root wires concrete Domain trait
//! objects into the router. The editor core routes opaque through contract
//! traits.

use reovim_lib_ds::{Bytes, Map, Seq};

// Contract types live in the Domain contract tier; re-exported here so callers
// that name `reovim_editor_core::router::*` continue to compile.
...
impl DomainRouter {
    ...
    ///
    /// ```rust
    /// use reovim_editor_core::router::DomainRouter;
    ///
    ...
    /// use reovim_editor_core::router::{DomainId, DomainRouter};
    ///
    /// let r = DomainRouter::new();
    ...
    /// let r = DomainRouter::new();
    /// use core::num::NonZeroU32;
    /// use reovim_editor_core::router::DomainId;
    /// let id = DomainId::new(NonZeroU32::new(1).unwrap());
    ...
    /// ```rust
    /// use core::num::NonZeroU32;
    /// use reovim_editor_core::router::{DomainId, DomainRouter, HandlerId};
    ///
    /// let r = DomainRouter::new();
    /// let id = DomainId::new(NonZeroU32::new(1).unwrap());
    ...
    /// ```rust
    /// use core::num::NonZeroU32;
    /// use reovim_editor_core::router::{DomainId, DomainRouter, ProjectorId};
    ///
    /// let r = DomainRouter::new();
    /// let id = DomainId::new(NonZeroU32::new(1).unwrap());
