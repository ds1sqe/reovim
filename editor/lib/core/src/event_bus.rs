//! `DS12EventBus` — the editor-core DS12 event fan-out bus (9.4, OBS1, CC6).
//!
//! ## Design
//!
...
//! any event is emitted (boot is `&mut EditorInit`, single-threaded) and is
...
/// Spec-default subscriber capacity per editor-core instance (9.4 §7).
///
/// Until the config service lands, the default is the capacity:
/// `editor.host.[limits].observe-subscriber-capacity` arrives with the
/// config service.
///
/// See [`DS12EventBus::subscribe`] for usage.
///
/// ```rust
/// use reovim_editor_core::event_bus::SUBSCRIBER_CAPACITY;
...
///
/// ```rust
/// use reovim_editor_core::event_bus::SubscribeError;
///
...
/// 9.4 §1 schema-view note: the editor core carries the same semantic information
/// as the schema-view fields without `HashMap`.
///
/// # Example
///
/// ```rust
/// use reovim_editor_core::event_bus::BootStageFields;
///
...
///
/// ```rust,no_run
/// use reovim_editor_core::event_bus::{DS12Event, BootStageFields};
/// use reovim_editor_core::BootClock;
...
///
/// ```rust
/// use reovim_editor_core::event_bus::EVT_BOOT_STAGE_START;
...
///
/// ```rust
/// use reovim_editor_core::event_bus::EVT_BOOT_STAGE_OK;
...
///
/// ```rust
/// use reovim_editor_core::event_bus::EVT_BOOT_STAGE_FAIL;
...
///
/// ```rust
/// use reovim_editor_core::event_bus::EVT_CARRIER_VALIDATION_ERROR;
///
...
///
/// ```rust
/// use reovim_editor_core::event_bus::{Subscriber, DS12Event};
///
...
/// The editor-core DS12 event fan-out bus (9.4, CC6).
///
...
///
/// Created by `DS12EventBus::new()` in `EditorInit::boot` before any stage runs.
/// Kept in `EditorCore::event_bus` after the handoff.
///
/// # Example
///
/// ```no_run
/// use reovim_editor_core::event_bus::{DS12EventBus, DS12Event, BootStageFields, EVT_BOOT_STAGE_START};
/// use reovim_editor_core::BootClock;
/// use reovim_uapi::abi::error::LogLevel;
///
...
pub struct DS12EventBus {
    ...
    ///
    /// Written once before any event is emitted (boot is `&mut EditorInit`,
...
impl DS12EventBus {
    ...
    /// until `set_builtin` is called. Called once in `EditorInit::boot` before stage 1.
    ///
    /// # Example
    ///
    /// ```rust
    /// use reovim_editor_core::event_bus::DS12EventBus;
    ///
    /// let bus = DS12EventBus::new();
    ...
    ///
    /// Called once during boot (under `&mut EditorInit`, single-threaded) before
    ...
    /// boot-stage-0 wiring call in `EditorInit::boot` is the intended caller
    ...
    /// use reovim_editor_core::event_bus::DS12EventBus;
    /// use reovim_editor_core::log::ring::LogRing;
    ///
    ...
    ///
    /// ```no_run
    /// use reovim_editor_core::event_bus::{DS12EventBus, DS12Event, Subscriber};
    ///
    ...
    ///
    /// ```no_run
    /// use reovim_editor_core::event_bus::{DS12EventBus, DS12Event, BootStageFields, EVT_BOOT_STAGE_OK};
    /// use reovim_editor_core::BootClock;
    /// use reovim_uapi::abi::error::LogLevel;
    ...
    pub fn emit(&self, event: &DS12Event) {
        ...
        //
        // `builtin_ring` is written once under `&mut EditorInit` (single-threaded
