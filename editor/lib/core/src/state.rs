//! EditorCore-owned Phase 3 state substrate (§3.2, §3.3, §3.4, §4.3).
//!
//! The contract tier defines portable value shapes. This module owns editor-core
...
///
/// ```rust
/// use reovim_editor_core::state::CarrierStateLimits;
///
/// let limits = CarrierStateLimits {
///     max_position_carrier_bytes: 1,
///     ..CarrierStateLimits::default()
/// };
/// assert_eq!(limits.max_position_carrier_bytes, 1);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct CarrierStateLimits {
...
impl Default for CarrierStateLimits {
    fn default() -> Self {
        Self::DEFAULT
    }
}

/// Resource limits enforced by the Phase 3 editor-core state substrate.
///
/// ```rust
/// use reovim_editor_core::state::EditorStateLimits;
///
/// let limits = EditorStateLimits::default();
/// assert!(limits.max_view_slot_bytes > 0);
/// ```
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct EditorStateLimits {
    ...
    /// EditorCore-wide service lease timeout.
    ...
    /// Maximum editor-core-owned stream queued bytes.
...
impl EditorStateLimits {
    /// Builds a limits record.
    ///
    /// ```rust
    /// use reovim_editor_core::state::EditorStateLimits;
    ///
    /// let limits = EditorStateLimits::new(8, 2, 100);
    ...
    pub const fn new(
        ...
        let carrier = CarrierStateLimits::DEFAULT;
        Self {
            max_view_slot_bytes,
    ...
    /// use reovim_editor_core::state::EditorStateLimits;
    ///
    /// let limits = EditorStateLimits::default().with_register_limits(1, 2, 3, 4);
    ...
    /// use reovim_editor_core::state::EditorStateLimits;
    ///
    /// let limits = EditorStateLimits::default().with_undo_limits(1, 2, 3);
    ...
    ///
    /// ```rust
    /// use reovim_editor_core::state::{CarrierStateLimits, EditorStateLimits};
    ///
    /// let carrier = CarrierStateLimits {
    ///     max_codecs_per_domain: 5,
    ///     ..CarrierStateLimits::default()
    /// };
    /// let limits = EditorStateLimits::default().with_carrier_limits(carrier);
    /// assert_eq!(limits.max_codecs_per_domain, 5);
    /// ```
    #[must_use]
    pub const fn with_carrier_limits(mut self, limits: CarrierStateLimits) -> Self {
    ...
    /// use reovim_editor_core::state::EditorStateLimits;
    ///
    /// let limits = EditorStateLimits::default().with_stream_limits(1, 2, 3, 4, 5);
...
impl Default for EditorStateLimits {
    fn default() -> Self {
        Self::new(4096, 64, 30_000)
    }
}

/// EditorCore state substrate backing Phase 3 fixtures.
///
/// ```rust,no_run
/// // no_run: requires arch allocator runtime.
/// use reovim_editor_core::state::StateSubstrate;
///
/// let state = StateSubstrate::new();
/// assert!(state.buffer_count() == 0);
/// ```
pub struct StateSubstrate {
    limits: EditorStateLimits,
    ...
    /// EditorCore-owned service registry rows and leases.
...
impl StateSubstrate {
    ...
    pub fn new_with_thread(thread: ThreadControl) -> Self {
        Self::with_limits_and_thread(EditorStateLimits::default(), thread)
    }
    ...
    /// ```rust,no_run
    /// // no_run: requires arch allocator runtime.
    /// use reovim_editor_core::state::{EditorStateLimits, StateSubstrate};
    ///
    /// let state = StateSubstrate::with_limits(EditorStateLimits::new(32, 1, 10));
    /// assert_eq!(state.limits().max_view_slot_bytes, 32);
    /// ```
    #[must_use]
    pub const fn with_limits(limits: EditorStateLimits) -> Self {
    ...
    /// ```
    #[must_use]
    pub const fn with_limits_and_thread(limits: EditorStateLimits, thread: ThreadControl) -> Self {
        Self {
            limits,
            buffers: Map::new(),
            registers: Map::new(),
    ...
    /// ```
    #[must_use]
    pub const fn limits(&self) -> EditorStateLimits {
        self.limits
    }
    ...
    pub(crate) fn register_position_codec(
        ...
        if max_content_bytes > self.limits.max_position_carrier_bytes {
            return Err("codec: content limit exceeds editor-core cap");
        }
...
impl CarrierKind {
    const fn max_content_bytes(self, limits: EditorStateLimits) -> usize {
        match self {
...
impl CodecRegistry {
    const fn new(limits: EditorStateLimits) -> Self {
        Self {
            limits,
            rows: Map::new(),
...
impl Default for UndoLimits {
    fn default() -> Self {
        EditorStateLimits::default().undo_limits()
    }
}
...
/// ```rust,no_run
/// // no_run: requires arch allocator runtime.
/// use reovim_editor_core::state::BufferRecord;
...
/// EditorCore service registry storage and lease state.
///
/// ```rust,no_run
/// // no_run: requires arch allocator runtime.
/// use reovim_editor_core::state::ServiceRegistry;
///
...
impl ServiceRegistry {
    ...
    pub fn register(
        ...
        // `current_id()` returns i64; a native thread id is a small positive integer
...
/// One service row retained by the editor-core registry.
///
/// ```rust,no_run
/// // no_run: requires arch allocator runtime.
/// ```
