...
use crate::{
    ...
    init::{
        EditorInit, LauncherArgs, last_panic_record, record_panic_state,
...
arch_test!(flush_update_after_push_writes_bytes, {
    ...
    let line = b"[    0.000001] editor-core boot: ok\n";
...
arch_test!(flush_tail_ends_with_newline, {
    reset_for_test();
    let ring = LogRing::try_new(1024 * 1024).unwrap();
    let line = b"[    0.000100] editor-core init: stage=1 ok\n";
    update_after_push(line, &ring);
    let tail = ring_tail();
...
arch_test!(flush_cursor_grows_after_each_append, {
    ...
    let line1 = b"[    0.001000] editor-core boot: stage=1\n";
    let line2 = b"[    0.002000] editor-core boot: stage=2\n";
    let line3 = b"[    0.003000] editor-core boot: stage=3\n";
...
arch_test!(flush_second_boot_does_not_error, {
    ...
    // EditorInit::boot under selftest accepts AlreadySet as success.
    let init = EditorInit::new(LauncherArgs::default());
    let result = init.boot();
    assert!(
        result.is_ok(),
        "second EditorInit::boot in selftest process must not error (AlreadySet-ok rule)"
    );
});
...
// Chosen constants:
//   filler = b"FILLER\n"  (7 bytes)
//   line   = b"[    0.000001] editor-core boot: overflow-fits\n"  (42 bytes)
...
arch_test!(flush_overflow_compaction_then_line_fits, {
    ...
    let filler = b"FILLER\n";
    let line = b"[    0.000001] editor-core boot: overflow-fits\n";
...
arch_test!(flush_overflow_line_too_large_after_compaction, {
    ...
    let line = b"[    0.000001] editor-core boot: overflow-drop-test\n";
