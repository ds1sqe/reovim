...
//! ## Emitter forms
//!
//! - EditorCore event (§3): bare subsystem name (`init`, `dispatch`, …). An
//!   editor-core address never contains `/`.
...
///
/// This is always derived by the editor core from dispatch context, never
/// caller-supplied (LOG3).
///
/// # Example
///
/// ```rust
/// use reovim_editor_core::log::render::EmitterAddress;
///
/// let k = EmitterAddress::EditorCore { subsystem: "init" };
/// let c = EmitterAddress::Cdylib { kind: "module", id: 7, vtable: 0 };
/// assert!(matches!(k, EmitterAddress::EditorCore { .. }));
/// assert!(matches!(c, EmitterAddress::Cdylib { .. }));
/// ```
#[derive(Debug, Clone, Copy)]
pub enum EmitterAddress {
    /// editor-core-owned subsystem — bare name (`init`, `dispatch`, `config`, …).
    ///
    /// A editor-core address never contains `/` (9.5 §3 disambiguation rule).
    EditorCore {
        /// The editor-core subsystem name (e.g. `"init"`, `"dispatch"`).
...
///
/// ```rust
/// use reovim_editor_core::log::render::InstanceAddress;
///
...
///
/// ```rust
/// use reovim_editor_core::log::render::{EmitterAddress, InstanceAddress, RenderInput};
/// use reovim_uapi::abi::error::LogLevel;
///
/// let input = RenderInput {
///     ts_nanos: 2_413_000,
///     emitter_pkg: "editor-core",
///     emitter_addr: EmitterAddress::EditorCore { subsystem: "init" },
...
pub struct RenderInput<'a> {
    ...
    ///
    /// For editor-core events this is `"editor-core"`; for cdylib events it is the
...
///
/// ```rust
/// use reovim_editor_core::log::render::RenderError;
///
...
///
/// ```no_run
/// use reovim_editor_core::log::render::{EmitterAddress, InstanceAddress, RenderInput, render_line};
/// use reovim_uapi::abi::error::LogLevel;
///
/// let input = RenderInput {
...
pub fn render_line(input: &RenderInput<'_>) -> Result<Bytes, RenderError> {
    ...
    match input.emitter_addr {
        EmitterAddress::EditorCore { subsystem } => {
...
/// Maps a dotted OBS1 event name to its editor-core subsystem label (9.5 §3).
///
/// The first dotted segment is the family; a static label is returned for the
/// families the boot-core crate emits. Unknown families fall back to `editor-core`.
fn editor_core_subsystem_from_event(event: &str) -> &'static str {
...
/// Renders an editor-core DS12 `event` into its canonical LOG2 line bytes.
///
...
pub(crate) fn render_event(event: &DS12Event) -> Result<Bytes, RenderError> {
    ...
    let input = RenderInput {
        ...
        emitter_pkg: "editor-core",
        emitter_addr: EmitterAddress::EditorCore {
            subsystem: editor_core_subsystem_from_event(event.event),
        },
        instance: InstanceAddress::None,
        message,
        level: event.level,
