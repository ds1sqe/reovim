...
//! ## Coverage
//!
//! - LOG2 golden: editor-core-event fixture renders byte-identical expected line.
...
// ── EditorCore-event golden (spec §2 worked example) ─────────────────────────────
//
...
arch_test!(log2_golden_editor_core_event, {
    ...
    let expected = "[    0.002413] editor-core init: boot.stage.ok stage=2 name=config\n";
    assert_eq!(
        got, expected,
        "LOG2 editor-core-event golden mismatch\n  got: {got:?}\n  expected: {expected:?}"
    );
});
...
arch_test!(log2_timestamp_large_seconds, {
    ...
    let input = RenderInput {
        ...
        emitter_pkg: "editor-core",
        emitter_addr: EmitterAddress::EditorCore { subsystem: "boot" },
        instance: InstanceAddress::None,
        message: "test",
...
arch_test!(log2_instance_none_omitted, {
    ...
    let bytes = render_line(&input).unwrap();
    let line = core::str::from_utf8(bytes.as_slice()).unwrap();
    // The line should be "[...] editor-core boot: test\n" — no instance token.
    let expected = "[    0.001000] editor-core boot: test\n";
...
arch_test!(render_alloc_fault_sweep_covers_error_arms, {
    ...
    let inputs = [
        ...
        RenderInput {
            ts_nanos: 3_000_000,
            emitter_pkg: "pkg",
            emitter_addr: EmitterAddress::EditorCore { subsystem: "log" },
...
arch_test!(render_alloc_fault_boundary_sweep, {
    ...
    let shapes = [
        (
            EmitterAddress::EditorCore { subsystem: "boot" },
        ...
        (
            EmitterAddress::EditorCore { subsystem: "log" },
            InstanceAddress::Client { id: 7 },
        ),
        (
            EmitterAddress::EditorCore { subsystem: "log" },
