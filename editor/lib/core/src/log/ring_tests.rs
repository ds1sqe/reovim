...
// sink state. This test runs EditorInit::boot and checks the ring has entries.

arch_test!(log6_ring_has_boot_stage_lines_before_sink, {
    // Use EditorInit::boot so the ring is wired through the normal path.
    use crate::init::{EditorInit, LauncherArgs};
    ...
    // tail emits 30 health-probe lines (15 ping-pong probes, each a
    // `probing...` line then a verdict). All must appear in the ring.
    assert!(editor_core.log_ring.len() > 0, "ring must have entries after EditorInit::boot");
    // 14 boot-stage events (7 × start+ok) + 30 boot-tail health lines = 44.
    assert_eq!(
        editor_core.log_ring.len(),
        44,
        "ring must hold 14 boot-stage events + 30 boot-tail health lines"
    );
});
...
// ── Fallback `editor-core` subsystem (ring.rs L329) ────────────────────────────────
//
// `editor_core_subsystem_from_event` returns `"editor-core"` for event names that do not
// start with `"boot."` or `"log."`. Push an event with a custom family name and
// assert that the rendered line contains the fallback subsystem field in the
// LOG2 format: `… editor-core editor-core …`.

arch_test!(push_event_fallback_subsystem_renders_editor_core, {
    let ring = LogRing::try_new(1024 * 1024).unwrap();
    ...
    let mut found_editor_core_subsystem = false;
    ring.for_each(|entry| {
        let line = core::str::from_utf8(entry.line.as_slice()).unwrap_or("");
        // LOG2 format: "[  ts] editor-core editor-core: custom.test\n"
        // The emitter_pkg is "editor-core" and the subsystem is "editor-core".
        if line.contains(" editor-core editor-core:") {
            found_editor_core_subsystem = true;
        }
    });
    assert!(
        found_editor_core_subsystem,
        "rendered line must use the fallback 'editor-core' subsystem for unknown event families"
    );
});
...
arch_test!(push_event_renders_enriched_boot_stage_line, {
    ...
    ring.for_each(|entry| {
        ...
        assert!(
            line.contains("editor-core boot: stage 2/7 ok: shell config"),
