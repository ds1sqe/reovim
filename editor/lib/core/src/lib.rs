//! `reovim-editor-core` — the editor-state core.
//!
//! This crate is the editor-core tier of the v0.16 sovereign rebuild (#778).
//! It sits above the product-facing `reovim-uapi` facade and target-neutral
//! `lib/*` crates in the layer DAG: no third-party crates, no `alloc`, every
//! heap container a `lib_ds::*` type (L10/DAG6).
//!
//! ## Design: two types, one handoff (2.1, LF13)
//!
//! Boot and steady state are different types, not different flags:
//!
//! - [`EditorInit`] exists only during boot. It exclusively owns the editor-core
//!   state under construction (`&mut self`, no locks), runs the boot stages
//!   (2.2 §1), and is *consumed* at the handoff.
//! - [`EditorCore`] is the steady-state root — inert shared state behind the
//!   hostapi. It can only be constructed by [`EditorInit::boot`] (LF13).
//!
...
//! | Module | Purpose |
//! |---|---|
//! | [`init`] | `EditorInit` typestate, `LauncherArgs`, `BootError` |
//! | [`root`] | `EditorCore` steady-state root (boot-core subset of 2.1 §3) |
...
// item inside is `pub` — nothing leaks past the crate. `EditorInit::boot` calls
...
// ── L12 sibling test files (editor-core-selftest bin in tests/fixtures/ runs these) ─
//
