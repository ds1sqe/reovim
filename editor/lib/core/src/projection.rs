...
//! `EditorCore::dispatch_input` returns a `Projection`, and ext Domains produce
//! one, so the type belongs to the contract that sits between them rather than
//! to the editor core itself. This module re-exports them so callers that name
//! `reovim_editor_core::projection::{Projection, ProjectionSpan, ProjectionDecodeError}`
//! continue to compile.
