//! `EditorInit` — the boot actor (2.1 §2, 2.2 §1, LF13).
//!
//! `EditorInit` exists only during boot. It exclusively owns the editor-core state under
//! construction (`&mut self`, no locks), drives the boot stages, and is
//! *consumed* by the handoff into `Shared<EditorCore>`. Using an `EditorInit` after
//! `boot()` is a compile error (the value is moved).
//!
//! Boot stages 0..7 (2.2 §1):
//!
//! ```text
//! stage 0: EditorInit::new  — LauncherArgs capture, BootClock allocation
//! stage 1: editor.host config materialised (CFG9)  — structural stub
//! stage 2: editor.shell config + runtime caps       — structural stub
...
//! stage 7: handoff — EditorInit::boot returns Shared<EditorCore>; serving
//! ```
//!
...
//! respective feature.
//!
//! On boot failure at any stage, `EditorInit` drops whole — no partially-constructed
//! editor-core state escapes (LF13).
...
use crate::{
    ...
    root::{EDITOR_CORE_ABI_VERSION, EditorCore, EditorCoreAbi},
    router::DomainRouter,
    session::{
...
/// Arguments parsed from the process entry point and passed into `EditorInit::new`.
///
...
///   freestanding targets without a DTB. The editor core stores this for later
...
///
/// ```rust
/// use reovim_editor_core::LauncherArgs;
///
...
///
/// When `EditorInit::boot` returns `Err(BootError)`, the `EditorInit` value has been
/// consumed — no partially-constructed editor-core state escapes (LF13). The
/// variant names the failing stage for diagnostic purposes.
///
/// # Example
///
/// ```rust
/// use reovim_editor_core::BootError;
///
...
pub enum BootError {
    ...
    /// Allocation failed during editor-core construction.
    Alloc,
...
/// use reovim_editor_core::init::record_panic_state;
...
const fn register_panic_config(
    ...
    match result {
        ...
        Err(PanicConfigError::AlreadyConfigured) => {
            ...
            // all tests; a prior EditorInit::boot already registered this seam with
...
///
/// Created by `EditorInit::new` and consumed by `EditorInit::boot`. While `EditorInit` exists,
/// the editor-core state is under exclusive mutable ownership (`&mut self`) — no
/// locks, no concurrent observers (LF13). Using an `EditorInit` after `boot()` is a
/// compile error.
///
/// # Example
///
...
/// let editor_core = init.boot().expect("boot must succeed in a healthy process");
/// let _ = editor_core; // Shared<EditorCore> — steady-state root
/// ```
pub struct EditorInit {
    /// Boot-only launcher arguments; die with `EditorInit` at the handoff (LF13).
...
impl EditorInit {
    ...
    ///
    /// This is the *only* `EditorInit` constructor. It allocates nothing — stage 0
    ...
    /// steady-state `Shared<EditorCore>`.
    ///
    /// This is the **only** `EditorCore` constructor (LF13). On success `EditorInit` is
    /// consumed and the boot-only fields (`args`) are dropped. On failure
    /// `EditorInit` drops whole — no partially-constructed editor-core state escapes.
    ///
    ...
    /// captures every `boot.stage.*` event. Both are transferred into `EditorCore`
    ...
    /// `EditorInit` has been consumed — caller retains no editor-core state.
    ///
    /// # Example
    ///
    /// ```rust,no_run
    ...
    ///
    /// let init = EditorInit::new(LauncherArgs::default());
    /// let editor_core = init.boot().expect("boot succeeds");
    /// drop(editor_core);
    /// ```
    pub fn boot(self) -> Result<Shared<EditorCore>, BootError> {
        ...
        // on `EditorCore` both keep the refcount ≥2 through the process lifetime.
        ...
        //
        // The composition root (and editor-core-selftest tests) register the text
        // Domain and attach a session AFTER boot by calling `EditorCore::setup_domain`
        // and `EditorCore::setup_session`. The editor core cannot depend on ext crates
        // (core/ext boundary), so `EditorInit::boot` provides empty containers.
        //
        ...
        // here is a stub; `EditorCore::setup_session` replaces it with a real one.
        // We use a sentinel domain_id (1) here; it is replaced immediately by
        // `EditorCore::setup_session` before any dispatch happens.
        ...
        let editor_core = EditorCore::new(crate::root::EditorCoreParts {
            abi_shared,
            boot_anchor,
        ...
        Shared::try_new(editor_core).map_err(|_| BootError::Alloc)
    }
}
