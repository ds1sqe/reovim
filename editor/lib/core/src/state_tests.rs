...
use crate::{
    ...
    state::{BufferRecord, CarrierStateLimits, EditorStateLimits, ServiceRegistry, StateSubstrate},
};
...
arch_test!(state_register_caps_enforce_scope_counts_and_carrier_bytes, {
    let limits = EditorStateLimits::new(16, 4, 100).with_register_limits(3, 1, 1, 1);
...
arch_test!(state_carrier_codecs_track_known_opaque_invalid_and_revalidation, {
    let limits = EditorStateLimits::new(16, 4, 100).with_carrier_limits(CarrierStateLimits {
...
arch_test!(state_stream_registry_enforces_init_window_and_lifecycle, {
    let limits = EditorStateLimits::new(16, 4, 100).with_stream_limits(1, 1, 16, 16, 100);
    let mut state = StateSubstrate::with_limits(limits);
...
arch_test!(state_stream_registry_queues_subscriptions_and_applies_byte_edits, {
    let limits = EditorStateLimits::new(16, 4, 100).with_stream_limits(4, 4, 64, 64, 100);
    let mut state = StateSubstrate::with_limits(limits);
    ...
    assert_eq!(
        state
            .stream_control_class(stream, StreamControlOp::EDITOR_CORE_DRAIN)
            .expect("editor-core op"),
        StreamControlClass::EditorCore
    );
    assert_eq!(
        state
...
arch_test!(state_view_slots_are_isolated_capped_and_dropped_by_owner, {
    let mut state = StateSubstrate::with_limits(EditorStateLimits::new(4, 1, 100));
    let client = ClientId::new(1);
...
arch_test!(buffer_record_undo_group_cap_drops_oldest_group, {
    let mut state = StateSubstrate::with_limits(
        EditorStateLimits::new(16, 4, 100).with_undo_limits(1, 16, 1024),
    );
...
arch_test!(phase3_editor_core_state_hostapi_integration_smoke, {
    ...
    {
        let mut session = editor_core.session.state.lock();
        session
    ...
    assert!(editor_core.hostapi_undo(buffer).expect("undo"));
    assert_eq!(
        editor_core
            .hostapi_buffer_bytes(buffer)
    ...
    assert_eq!(
        editor_core.hostapi_service_lease_validate(lease.id, 20),
        Err(ServiceAccessError::Stale)
    );
});

arch_test!(phase4_editor_core_carrier_and_stream_hostapi_integration_smoke, {
    let editor_core = EditorInit::new(LauncherArgs::default())
        .boot()
        .expect("boot succeeds");
    ...
    assert!(
        editor_core
            ...
            .is_err(),
        "invalid carrier is rejected at editor-core ingress"
    );
    ...
    assert_eq!(
        editor_core
            .hostapi_stream_control_class(stream, StreamControlOp::EDITOR_CORE_INSPECT)
