...
//!
//! The `session_dispatch_integration_smoke` test boots an editor core, registers
...
//! compose end-to-end inside the editor core before any wire is involved.
//!
...
//! the editor-core crate does NOT have a runtime dep on the text Domain crate (the
//! core/ext boundary is maintained). The editor-core-selftest fixture bin enables
...
// ── Integration smoke (Phase 2 AC) ───────────────────────────────────────────
//
// The editor-core-level smoke exercises session + router mechanics without any ext
// dependency. The full end-to-end integration smoke (editor core + text Domain
// handler/projector + dispatch) lives in the editor-core-selftest bin's own module
// (`tests/fixtures/editor-core-selftest/src/domain_smoke.rs`), where the bin can
// link `reovim-domain-text` without adding an ext dep to the editor-core rlib.
//
// This test verifies the dispatch path returns the expected error when no
// handler is registered (the sentinel session state installed by EditorInit::boot).
...
arch_test!(session_dispatch_without_domain_returns_err, {
    ...
    let result = editor_core.dispatch_input(b"x");
