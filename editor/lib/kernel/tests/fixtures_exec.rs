//! Integration tests that build the `#![no_std] #![no_main]` kernel fixtures,
...
//! the `no_std` test runner is the `kernel-selftest` bin. This file only
...
//! is pinned to native host execution. The kernel fixtures link `reovim-kernel`
...
// ---------------------------------------------------------------------------
// Serialization guard for the kernel-selftest tests
//
// `kernel_selftest_all_pass_exits_zero` and
// `kernel_selftest_inject_failure_exits_nonzero` both build package
// `kernel-selftest` with DIFFERENT feature sets (`[]` vs `["inject-failure"]`),
...
///
/// The fixtures live in a nested workspace (`server/lib/kernel/tests/fixtures/`)
...
/// `KERNEL_FIXTURE_TARGET` / `KERNEL_FIXTURE_RUNNER` env var handling here,
...
fn build_fixture_features(pkg: &str, features: &[&str]) -> PathBuf {
    // `CARGO_MANIFEST_DIR` is `server/lib/kernel/` (the crate under test);
    // the nested fixture workspace lives at
    // `server/lib/kernel/tests/fixtures/`.
...
fn scratch_path(tag: &str) -> PathBuf {
    ...
    p.push(format!("reovim-kernel-fixture-{tag}-{}.log", std::process::id()));
    p
}
...
#[test]
fn kernel_selftest_all_pass_exits_zero() {
    // The kernel test-suite charter smoke (pass side): all kernel `*_tests.rs`
    // tests run on the in-repo no_std runner and exit 0 when every test passes.
    //
    // Serialized against `kernel_selftest_inject_failure_exits_nonzero` via
    ...
    let exe = build_fixture("kernel-selftest");
    assert_eq!(run(&exe, &[]), 0, "all-pass kernel test run exits 0");
}

#[test]
fn kernel_selftest_inject_failure_exits_nonzero() {
    // The kernel test-suite charter smoke (fail side): the inject-failure
    ...
    //
    // Serialized against `kernel_selftest_all_pass_exits_zero` via
    // `selftest_lock()` — see the guard's comment for the shared-artifact
    // hazard. The guard spans build AND exec.
    let _guard = selftest_lock();
    let exe = build_fixture_features("kernel-selftest", &["inject-failure"]);
    assert_eq!(run(&exe, &[]), 70, "inject-failure exits the halt code");
}

#[test]
fn kernel_panic_halt_flushes_log2_and_exits_70() {
    // AB12 integration smoke (halt path): the fixture boots the kernel,
    ...
    let exe = build_fixture("kernel-panic-halt");
    ...
    assert_log2_kernel_panic(&out, "halt-path kernel fixture panic");
}

#[test]
fn kernel_panic_recover_flushes_log2_and_exits_75() {
    // AB12 integration smoke (recover path): same structure as the halt
    // fixture but with Disposition::Recover — exits 75 (EX_TEMPFAIL).
    //
    // AC (c) split: same reasoning as `kernel_panic_halt_flushes_log2_and_exits_70`.
    // The `init_tests.rs` selftest suite covers the state-record hook in-process.
    let exe = build_fixture("kernel-panic-recover");
    let sink = scratch_path("recover");
    let code = run(&exe, &[sink.to_str().unwrap()]);
    let out = read_and_remove(&sink);
    assert_eq!(code, 75, "recover disposition exits EX_TEMPFAIL (75)");
    assert_has_log2_lines(&out);
    assert_log2_kernel_panic(&out, "recover-path kernel fixture panic");
}

// ---------------------------------------------------------------------------
// No-catch_unwind assertion (AB12: no `catch_unwind` in kernel)
// ---------------------------------------------------------------------------

#[test]
fn no_catch_unwind_in_kernel_src() {
    // DAG6: no unwinder; AB12 — a panic reaches arch's handler directly.
    // `catch_unwind` in the kernel crate would be unsound (no unwinder to
    // unwind through). Walk `server/lib/kernel/src/` and fail if any `.rs`
    ...
    let violations = walk_rs_files_for_pattern(&kernel_src, "catch_unwind");
    assert!(
        violations.is_empty(),
        "kernel/src contains catch_unwind (DAG6/AB12 violation): {violations:?}"
    );
}
