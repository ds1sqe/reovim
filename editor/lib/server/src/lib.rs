...
//!
//! Boot stage 6 in `server/lib/kernel/src/boot.rs` runs as a structural stub
//! — it emits `boot.stage.{start,ok}` and performs no policy work during the
//! kernel `Init::boot` call. **The composition root fills the stage-6 body
//! by calling [`start_listener`] after `Init::boot` returns**, keeping the
//! `ServerKernel → ServerRuntime` dependency direction illegal (the kernel
...
//! `Init::boot`. The comment in `boot.rs` stage 6 already says
