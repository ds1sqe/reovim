...
use crate::{
    ...
    testing::{MockPlatformCapabilities, MockServerHandle, MockThemeProvider},
    traits::{ClientModule, ModuleContext, PlatformCapabilities, ThemeProvider},
    types::{ClientModuleError, ProbeResult, Version},
};
...
fn make_ctx() -> ModuleContext<'static> {
    let caps: &'static dyn PlatformCapabilities =
        Box::leak(Box::new(MockPlatformCapabilities::new()));
    let theme: &'static dyn ThemeProvider = Box::leak(Box::new(MockThemeProvider::new()));
    ModuleContext {
        capabilities: caps,
        server: Arc::new(MockServerHandle::new()),
        theme,
        services: None,
