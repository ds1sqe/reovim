...
- **`platform/`** (`reovim-kabi-platform`) — the `#[repr(C)]` platform handle
  vtable (clock, alloc, dealloc, park, unpark), the process-global write-once
  handle install, and `AllocError`. `arch` implements it (`arch → kabi`).
...
- **`device/`** — the device contract. **Not a crate this flight.** Only the
  ...
  workspace member. (SP02 settled decision 2.)
