...
//!
//! [`BufferApi`]: reovim_driver_text_session::api::BufferApi
...
use {
    libc::c_char, reovim_domain_text::Position, reovim_driver_text_session::api::BufferApi,
...
pub unsafe extern "C" fn reovim_delete_buffer(buffer_id: ReovimBufferId) -> i32 {
    ffi_catch_unwind(AssertUnwindSafe(|| {
        ...
        match with_runtime(|rt| rt.delete_buffer(bid)) {
            Err(e) => e,
            Ok(Ok(())) => REOVIM_OK,
            Ok(Err(reovim_driver_text_session::api::BufferError::NotFound(_))) => {
                REOVIM_ERR_NOT_FOUND
            }
            Ok(Err(reovim_driver_text_session::api::BufferError::CannotDeleteLastBuffer)) => {
