...
fn test_resolve_with_session_yy_with_count_uses_cursor() {
    ...
    }) = result
    {
        ...
        assert_eq!(
            args.get("range_start"),
            Some(&reovim_subsys_command_types::ArgValue::Position(1, 0))
        );
        assert_eq!(
            args.get("range_end"),
...
fn test_on_command_complete_with_textobj_range() {
    ...
    }) = result
    {
        ...
        assert_eq!(
            args.get("range_end"),
            Some(&reovim_subsys_command_types::ArgValue::Position(0, 7))
        );
...
fn test_on_command_complete_with_linewise_textobj() {
    ...
    }) = result
    {
        ...
        if let Some(reovim_subsys_command_types::ArgValue::Register(r)) = args.get("register") {
            assert_eq!(*r, 'a');
        } else {
...
fn test_on_command_complete_backward_motion_normalizes_range() {
    ...
    }) = result
    {
        ...
        assert_eq!(
            args.get("range_end"),
            Some(&reovim_subsys_command_types::ArgValue::Position(0, 10))
        );
    } else {
        panic!("expected Pop with ExecuteCommand, got {result:?}");
    }
...
fn test_on_command_complete_with_register_and_count() {
    ...
    {
        assert_eq!(args.get("count"), Some(&reovim_subsys_command_types::ArgValue::Count(2)));
        if let Some(reovim_subsys_command_types::ArgValue::Register(r)) = args.get("register") {
            assert_eq!(*r, 'b');
        } else {
