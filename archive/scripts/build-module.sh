...
get_cdylib_modules() {
    local modules=()
    for cargo_toml in "$REPO_ROOT"/ext/server/modules/*/Cargo.toml; do
...
get_package_name() {
    local module=$1
    local cargo_toml="$REPO_ROOT/ext/server/modules/$module/Cargo.toml"
...
build_module() {
    ...
    local cargo_toml="$REPO_ROOT/ext/server/modules/$module/Cargo.toml"
    if [[ ! -f "$cargo_toml" ]]; then
        echo -e "${RED}Error: Module '$module' not found at ext/server/modules/$module/${NC}" >&2
        return 1
    fi
