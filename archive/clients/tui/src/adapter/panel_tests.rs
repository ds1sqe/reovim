use reovim_driver_display::BufferId;

use super::*;

fn test_panel() -> TuiPanel {
    TuiPanel::new(BufferId::from_raw(42), 100, 24, 1000)
}
...
fn test_panel_visible_range_small_buffer() {
    let panel = TuiPanel::new(BufferId::from_raw(42), 100, 24, 10);
...
fn test_panel_scroll_to_near_bottom() {
    ...
    assert_eq!(panel.scroll_top, 976);
}

#[test]
fn test_panel_cursor_position() {
    let mut panel = test_panel();
    panel.cursor_line = 15;
    panel.cursor_col = 8;
...
fn test_panel_set_cursor() {
    ...
    assert_eq!(panel.cursor_line, 25);
    assert_eq!(panel.cursor_col, 10);
}
