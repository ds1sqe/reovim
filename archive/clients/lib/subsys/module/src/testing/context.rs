...
use crate::{
    ...
    traits::{ClientModuleRegistry, ModuleContext, ServerHandle},
};
...
impl TestModuleContext {
    ...
    pub fn as_context(&self) -> ModuleContext<'_> {
        ModuleContext {
            ...
            server: Arc::clone(&self.server) as Arc<dyn ServerHandle>,
            theme: &self.theme,
            services: self.services.as_ref(),
            module_registry: self
                .registry
                .as_ref()
                .map(|r| r as &dyn ClientModuleRegistry),
        }
    }
...
impl TestModuleContextBuilder {
    ...
    pub fn server_options(
        mut self,
        options: std::collections::HashMap<String, crate::types::OptionValue>,
    ) -> Self {
    ...
    #[must_use]
    pub fn highlight(mut self, group: &str, style: crate::types::Style) -> Self {
