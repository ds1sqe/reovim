...
//!
//! Mirror of `reovim-client-subsys-driver-loader::error`, retargeted at
//! server-side driver families. The variant set is identical because
//! the failure modes ([`LoadError::LibraryOpen`], [`LoadError::Validation`],
//! [`LoadError::DriverError`], [`LoadError::DriverPanicked`],
//! [`ScanEntryError::Loader`] / [`ScanEntryError::AbiMismatch`] on scan)
//! are inherent to the cdylib loading + ABI-validation pipeline and do
//! not vary by driver tier.
...
/// [`crate::LoadedNetGrpc::from_path_scan`].
///
