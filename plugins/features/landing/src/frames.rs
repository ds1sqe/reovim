...
mod tests {
    ...
    #[test]
    fn test_size_selection_large() {
        assert_eq!(LogoSize::from_dimensions(80, 24), LogoSize::Large);
        assert_eq!(LogoSize::from_dimensions(50, 24), LogoSize::Large);
    }

    #[test]
    fn test_size_selection_medium() {
        assert_eq!(LogoSize::from_dimensions(40, 18), LogoSize::Medium);
        assert_eq!(LogoSize::from_dimensions(49, 23), LogoSize::Medium);
    }

    #[test]
    fn test_size_selection_small() {
        assert_eq!(LogoSize::from_dimensions(20, 10), LogoSize::Small);
        assert_eq!(LogoSize::from_dimensions(34, 15), LogoSize::Small);
    }

    #[test]
    fn test_frames_not_empty() {
        assert!(!LARGE_ROAR_FRAMES.is_empty());
        assert!(!MEDIUM_SLEEP_FRAMES.is_empty());
        assert!(!SMALL_BREATHING_FRAMES.is_empty());
    }

    #[test]
    fn test_frames_consistent_widths() {
        // Each frame in a set should have consistent line count
        for frames in LARGE_ROAR_FRAMES {
            assert_eq!(frames.len(), LARGE_FRAME_0.len());
        }
        for frames in MEDIUM_SLEEP_FRAMES {
            assert_eq!(frames.len(), MEDIUM_FRAME_0.len());
        }
        for frames in SMALL_BREATHING_FRAMES {
            assert_eq!(frames.len(), SMALL_FRAME_0.len());
        }
    }
}
