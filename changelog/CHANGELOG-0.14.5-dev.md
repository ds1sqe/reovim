...
- **BREAKING (internal)**: `reovim-domain-text`, `reovim-domain-text-events`, and `reovim-provider-text` removed from `reovim-server` entirely (both `[dependencies]` and `[dev-dependencies]`). Server is now a domain-agnostic dispatch layer with zero domain crate imports in production and test code. All text-domain types accessed via driver re-exports (`reovim-driver-session`, `reovim-driver-buffer`, `reovim-driver-codec`). `insert_char_for_client` deleted, `FallbackContext` impl removed from `AppState`, gRPC serialization decoupled (`yank_type_str()`, `line_to_byte_offset()`, `text_edit_to_decoded_edit` moved to driver-codec), syntax event conversion moved to driver-syntax. Depgraph guard test `server_no_domain_text` checks all dependency kinds (normal + dev)
...
- **server**: add `ProjectionUpdatedPayload` to proto v2 notifications, `build_projection_notifications()` for generic opaque projection delivery, wire post-dispatch projection collection into the notification pipeline; deprecate `is_insert` string matching in `ModeChangedPayload`; remove `SelectionMode` domain-variant matching from notification builder and state handler (#753)
- **server**: fix server compilation after C/D chain — delete typed notification builders (mode, cursor, selection, viewport, option), delete typed state RPCs (get_mode, get_cursor, get_selection), add projection-based RPCs (get_projections, debug_get_projections), rename RequiresCursorSync → RequiresDomainSync, migrate CLI client and testing library to projection-based queries (#753)
- **protocol**: proto v3 big-bang — delete all typed display messages (Position, Selection, CursorMoved, ModeChanged, SelectionChanged, ViewportUpdated, OptionChanged), replace with domain-neutral `DomainDatum`/`ProjectionUpdatedPayload`; add `GetProjections`/`DebugGetProjections` RPCs; `RegisterEntry` uses `DomainDatum`; `ClientPresence` uses `optional DomainDatum viewport_state` (#753)
- **subsys**: strip text-domain types from subsys layer — `Viewport` moved to driver, `CursorSnapshot` becomes opaque `[u8;8]`, `DomainStateQuery`/`SelectionInfo`/`RegisterInfo` deleted, `ChangeSet` removed from public API (`DomainDriver` returns `DispatchResult`/`CommandResult`), `View`/`ViewManager`/`LineIndex`/`ColIndex` deleted from subsys-layout, `Anchor::Cursor` replaced with `Anchor::CursorInWindow(WindowId)`, `BufferContentProvider::display_lines` uses `(offset, count)` scalars; add `SurfaceDescriptor`, `LayoutTopology`/`Permil`/`SplitError`, `RootCompositor::generation()`/`topology()` (#753)
