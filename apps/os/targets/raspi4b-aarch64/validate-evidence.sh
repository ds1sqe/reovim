...
require_re '^- \[[xX]\] `dump status` / `dump snapshot` / `dump sync` expose dump state, image identity, boot/device/proof/panic sections, retained last-sync state, and fail-closed persistence state\.$' 'dump fail-closed checkbox'
require_re '^- \[[xX]\] `halt` attempts dump sync before root daemon shutdown\.$' 'halt dump-sync checkbox'
require_re '^- \[[xX]\] `cat /log/events` reports structured kernel events with process/task identity\.$' 'klog structured events checkbox'
require_re '^- \[[xX]\] `proc` reports the live process table through a `/bin` process-management program\.$' 'proc process table checkbox'
require_re '^- \[[xX]\] `proc` reports `loader=linked-bin entry_fn=bin_proc` for the running `/bin/proc` process\.$' 'proc loader metadata checkbox'
...
require_re '^- \[[xX]\] `dmesg` contains `shell\.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=<command>` audit lines for the typed commands\.$' 'dmesg /bin/sh session audit checkbox'
require_re '^- \[[xX]\] `dmesg` contains `exec\.parent path=/bin/<command> \.\.\. ppid=2 \.\.\. parent_task=2` audit lines for shell-launched `/bin` programs\.$' 'dmesg shell parentage audit checkbox'
...
require_re '^- \[[xX]\] `help pwd` / `help ls` / `help cd` / `help cat` / `help read` / `help mount` make VFS navigation and TTY line reads self-describing\.$' 'targeted VFS/read help checkbox'
...
require_re '^- \[[xX]\] `help device` / `help launch` / `help reovim` / `help hello` / `help halt` make boot inventory, payload, linked-bin, and shutdown `/bin` programs self-describing\.$' 'remaining program help checkbox'
...
require_count_at_least '^package=reovim-os$' 'package through image, status, and dump surfaces' 5
require_count_at_least '^version=[0-9A-Za-z.+_-]+$' 'version through image, status, and dump surfaces' 5
require_count_at_least '^target=aarch64-unknown-none$' 'target through image, status, and dump surfaces' 5
require_count_at_least '^selected_profile=shell-only$' 'selected profile through image, status, and dump surfaces' 5
require_count_at_least '^profile_request=shell-only$' 'profile request through image, status, and dump surfaces' 5
require_count_at_least '^launch_profile_feature=disabled$' 'launch-profile feature through image, status, and dump surfaces' 5
...
require_re '^cat \[path\.\.\.\] - print stdin or kernel VFS pseudo files$' 'targeted cat help output'
require_re '^reovim-os> help read$' 'targeted read help command in pasted transcript'
require_re '^read - read one TTY line$' 'targeted read help output'
...
require_re '^reovim-os> help dump$' 'targeted dump help command in pasted transcript'
require_re '^dump \[status\|snapshot\|sync\] - inspect or flush kernel dump state$' 'targeted dump help output'
require_re '^reovim-os> help sched$' 'targeted sched help command in pasted transcript'
require_re '^sched \[status\|tick\|yield\|sleep TICKS\] - inspect scheduler state, tick, yield, or sleep current task$' 'targeted sched help output'
require_re '^reovim-os> help proc$' 'targeted proc help command in pasted transcript'
require_re '^proc \[processes\|execs\|address-spaces\|page-tables\|pages\|memory-objects\|media\|pending\|self\|session\|services\|sources\|tasks\|waits\|syscalls\|continuations\|scheduler\] - inspect process state$' 'targeted proc help output'
...
require_re '^reovim-os> help hello$' 'targeted hello help command in pasted transcript'
require_re '^hello - print a linked-bin syscall proof$' 'targeted hello help output'
require_re '^reovim-os> hello$' 'linked hello command in pasted transcript'
require_re '^hello from linked bin$' 'linked hello output'
...
require_re '^/bin programs: help, init, sh, clear, screentest, pwd, ls, cd, cat, read, mount, input, status, proof, device, dmesg, dump, sched, proc, probe, launch, reovim, hello, halt, ps, kill, wake, block, spawn, sleep, wait, wait-ticks, exec, service-stop, service-start, service-restart, session, services, tasks, waits, syscalls, continuations, execs, pending, sources, media, self, install-bin, install-payload, install-bin-media, install-payload-media$' 'help /bin program catalog'
require_count_at_least '^/bin programs: help, init, sh, clear, screentest, pwd, ls, cd, cat, read, mount, input, status, proof, device, dmesg, dump, sched, proc, probe, launch, reovim, hello, halt, ps, kill, wake, block, spawn, sleep, wait, wait-ticks, exec, service-stop, service-start, service-restart, session, services, tasks, waits, syscalls, continuations, execs, pending, sources, media, self, install-bin, install-payload, install-bin-media, install-payload-media$' 'help /bin program catalog through command and VFS' 2
...
require_re '^  help \[program\] - show program help$' 'VFS help detailed help row'
require_re '^  init - start userland session services$' 'VFS help detailed init row'
...
require_re '^  cat \[path\.\.\.\] - print stdin or kernel VFS pseudo files$' 'VFS help detailed cat row'
require_re '^  read - read one TTY line$' 'VFS help detailed read row'
...
require_re '^  dump \[status\|snapshot\|sync\] - inspect or flush kernel dump state$' 'VFS help detailed dump row'
require_re '^  sched \[status\|tick\|yield\|sleep TICKS\] - inspect scheduler state, tick, yield, or sleep current task$' 'VFS help detailed sched row'
require_re '^  proc \[processes\|execs\|address-spaces\|page-tables\|pages\|memory-objects\|media\|pending\|self\|session\|services\|sources\|tasks\|waits\|syscalls\|continuations\|scheduler\] - inspect process state$' 'VFS help detailed proc row'
require_re '^  exec PROGRAM \[ARG\.\.\.\] - replace current process image$' 'VFS help detailed exec row'
...
require_re '^  hello - print a linked-bin syscall proof$' 'VFS help detailed hello row'
...
require_re '^/bin programs:$' 'proof /bin program-list header'
...
require_re '^  help read$' 'proof checklist targeted read help command'
...
require_re '^  help dump$' 'proof checklist targeted dump help command'
require_re '^  help sched$' 'proof checklist targeted sched help command'
require_re '^  help proc$' 'proof checklist targeted proc help command'
...
require_re '^  help hello$' 'proof checklist targeted hello help command'
require_re '^  hello$' 'proof checklist linked hello command'
...
require_re '^  dump status$' 'proof checklist dump status command'
require_re '^  dump snapshot$' 'proof checklist dump snapshot command'
require_re '^  dump sync$' 'proof checklist dump sync command'
require_re '^  cat /log/stats$' 'proof checklist VFS log stats command'
require_re '^  cat /log/events$' 'proof checklist VFS log events command'
require_re '^  proc$' 'proof checklist proc command'
...
require_re '^  dmesg contains shell\.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=<command>$' 'proof expected /bin/sh session dmesg fact'
require_re '^  dmesg contains exec\.parent path=/bin/<command> ppid=2 parent_task=2$' 'proof expected shell parentage dmesg fact'
...
require_re '^  kernel structured events available through /log/events$' 'proof expected log events fact'
require_re '^  dump status available through /bin/dump$' 'proof expected dump status fact'
require_re '^  dump sync fails closed until persistent storage is available$' 'proof expected dump sync fail-closed fact'
require_re '^  halt attempts dump sync before root daemon shutdown$' 'proof expected halt dump-sync fact'
...
require_re '^  shell\.status=error for disabled payload programs$' 'proof expected payload-disabled status fact'
...
require_re '^reovim-os> dump status$' 'dump status command in pasted transcript'
require_re '^format=reovim-dump-v1$' 'dump status format'
require_re '^package=reovim-os$' 'dump status package identity'
require_re '^target=aarch64-unknown-none$' 'dump status target identity'
require_re '^selected_profile=shell-only$' 'dump status selected profile'
require_re '^persistent=unavailable$' 'dump status persistent unavailable'
require_re '^last_sync_attempted=false$' 'dump status initial last-sync attempted flag'
require_re '^last_sync_reason=never-synced$' 'dump status initial last-sync reason'
require_re '^reovim-os> dump snapshot$' 'dump snapshot command in pasted transcript'
require_re '^reovim-dump-v1$' 'dump snapshot header'
require_re '^profile_request=shell-only$' 'dump snapshot profile request'
require_re '^launch_profile_feature=disabled$' 'dump snapshot launch feature identity'
require_re '^last_sync_attempted=false$' 'dump snapshot initial last-sync attempted flag'
require_re '^last_sync_reason=never-synced$' 'dump snapshot initial last-sync reason'
require_re '^checksum=[0-9]+$' 'dump snapshot checksum'
require_re '^dump-sync:$' 'dump snapshot dump-sync section'
require_re '^reovim-os> dump sync$' 'dump sync command in pasted transcript'
require_re '^attempted=true$' 'dump sync attempted flag'
require_re '^status=not-written$' 'dump sync not-written status'
require_re '^reason=no-persistent-dump-sink$' 'dump sync fail-closed reason'
...
require_re '^retained_events=[1-9][0-9]*$' 'klog stats retained event count line'
require_re '^reovim-os> cat /log/events$' 'VFS log events command in pasted transcript'
require_re '^events:$' 'structured event table header'
require_re '^- seq=[1-9][0-9]* component=proc severity=info kind=program-exit pid=[1-9][0-9]* task=[1-9][0-9]*$' 'structured process-exit event row'
require_re '^reovim-os> proc$' 'proc command in pasted transcript'
require_re '^processes:$' 'proc process table header'
require_re 'path=rootd' 'proc includes rootd process'
require_re 'path=/bin/sh' 'proc includes /bin/sh session process'
require_re 'path=/bin/proc' 'proc includes current proc process'
require_re 'path=rootd exit=0 loader=kernel entry_fn=rootd_main' 'proc includes rootd loader metadata'
require_re 'path=/bin/sh exit=0 loader=linked-bin entry_fn=bin_sh' 'proc includes /bin/sh loader metadata'
require_re 'path=/bin/proc exit=0 loader=linked-bin entry_fn=bin_proc' 'proc includes current proc loader metadata'
...
require_usb_source_for_shell_command 'help read' 'shell: help read'
...
require_usb_source_for_shell_command 'help dump' 'shell: help dump'
require_usb_source_for_shell_command 'help sched' 'shell: help sched'
require_usb_source_for_shell_command 'help proc' 'shell: help proc'
require_usb_source_for_shell_command 'help hello' 'shell: help hello'
require_usb_source_for_shell_command 'hello' 'shell: hello'
...
require_usb_source_for_shell_command 'dump status' 'shell: dump status'
require_usb_source_for_shell_command 'dump snapshot' 'shell: dump snapshot'
require_usb_source_for_shell_command 'dump sync' 'shell: dump sync'
require_usb_source_for_shell_command 'cat /log/stats' 'shell: cat /log/stats'
require_usb_source_for_shell_command 'cat /log/events' 'shell: cat /log/events'
require_usb_source_for_shell_command 'proc' 'shell: proc'
...
require_re '^shell\.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=proof$' 'dmesg /bin/sh session audit for proof'
require_re '^shell\.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=proc$' 'dmesg /bin/sh session audit for proc'
require_re '^shell\.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=cat /log/dmesg$' 'dmesg /bin/sh session audit for cat /log/dmesg'
require_re '^exec\.parent path=/bin/proof pid=[1-9][0-9]* ppid=2 task=[1-9][0-9]* parent_task=2$' 'dmesg shell parentage audit for proof'
require_re '^exec\.parent path=/bin/proc pid=[1-9][0-9]* ppid=2 task=[1-9][0-9]* parent_task=2$' 'dmesg shell parentage audit for proc'
require_re '^exec\.parent path=/bin/cat pid=[1-9][0-9]* ppid=2 task=[1-9][0-9]* parent_task=2$' 'dmesg shell parentage audit for cat /log/dmesg'
...
require_re '^shell: help read$' 'dmesg audit for targeted read help'
...
require_re '^shell: help dump$' 'dmesg audit for targeted dump help'
require_re '^shell: help sched$' 'dmesg audit for targeted sched help'
require_re '^shell: help proc$' 'dmesg audit for targeted proc help'
require_re '^shell: help hello$' 'dmesg audit for targeted hello help'
require_re '^shell: hello$' 'dmesg audit for linked hello'
...
require_re '^shell: dump status$' 'dmesg audit for dump status'
require_re '^shell: dump snapshot$' 'dmesg audit for dump snapshot'
require_re '^shell: dump sync$' 'dmesg audit for dump sync'
require_re '^shell: cat /log/stats$' 'dmesg audit for VFS log stats'
require_re '^shell: cat /log/events$' 'dmesg audit for VFS log events'
require_re '^shell: proc$' 'dmesg audit for proc'
