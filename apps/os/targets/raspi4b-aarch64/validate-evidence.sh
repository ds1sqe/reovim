...
require_re '^- \[[xX]\] `cd /dev` / `cat uart0` prove relative VFS device-file access\.$' 'relative device-file checkbox'
...
require_re '^reovim root shell$' 'help root-shell banner'
...
require_re '^reovim-os> pwd$' 'pwd command in pasted transcript'
...
require_re '^  pwd$' 'proof checklist pwd command'
...
require_count_at_least '^shell\.status=ok$' 'successful shell status audit lines' 27
