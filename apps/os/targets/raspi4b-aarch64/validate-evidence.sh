...
require_re '^- \[[xX]\] `dump status` / `dump snapshot` / `dump sync` expose dump state, image identity, boot/device/proof/panic sections, and fail-closed persistence state\.$' 'dump fail-closed checkbox'
...
require_re '^- \[[xX]\] `proc` reports `loader=source-image entry_fn=bin_proc` for the running `/bin/proc` process\.$' 'proc loader metadata checkbox'
...
require_re '^- \[[xX]\] `help device` / `help launch` / `help reovim` / `help halt` make boot inventory, payload, and shutdown `/bin` programs self-describing\.$' 'remaining program help checkbox'
...
require_re '^sched \[status\|tick\|yield\] - inspect scheduler state, tick, or yield current task$' 'targeted sched help output'
require_re '^reovim-os> help proc$' 'targeted proc help command in pasted transcript'
require_re '^proc \[processes\|execs\|media\|pending\|self\|sources\|tasks\|waits\|syscalls\|scheduler\|exec PROGRAM \[ARG\.\.\.\]\|spawn PROGRAM \[ARG\.\.\.\]\|block PROGRAM \[ARG\.\.\.\]\|wait PID\|wake PID\|kill PID\|install-bin NAME ok\|error\|install-payload NAME ready\|failed\|install-bin-media NAME\|install-payload-media NAME\] - inspect process state$' 'targeted proc help output'
...
require_re '^/bin programs: help, clear, screentest, pwd, ls, cd, cat, read, mount, input, status, proof, device, dmesg, dump, sched, proc, probe, launch, reovim, halt$' 'help /bin program catalog'
require_count_at_least '^/bin programs: help, clear, screentest, pwd, ls, cd, cat, read, mount, input, status, proof, device, dmesg, dump, sched, proc, probe, launch, reovim, halt$' 'help /bin program catalog through command and VFS' 2
...
require_re '^  sched \[status\|tick\|yield\] - inspect scheduler state, tick, or yield current task$' 'VFS help detailed sched row'
require_re '^  proc \[processes\|execs\|media\|pending\|self\|sources\|tasks\|waits\|syscalls\|scheduler\|exec PROGRAM \[ARG\.\.\.\]\|spawn PROGRAM \[ARG\.\.\.\]\|block PROGRAM \[ARG\.\.\.\]\|wait PID\|wake PID\|kill PID\|install-bin NAME ok\|error\|install-payload NAME ready\|failed\|install-bin-media NAME\|install-payload-media NAME\] - inspect process state$' 'VFS help detailed proc row'
...
require_re 'path=root-shell' 'proc includes root-shell process'
...
require_re 'path=root-shell exit=0 loader=kernel entry_fn=root_shell' 'proc includes root-shell loader metadata'
require_re 'path=/bin/proc exit=0 loader=source-image entry_fn=bin_proc' 'proc includes current proc loader metadata'
