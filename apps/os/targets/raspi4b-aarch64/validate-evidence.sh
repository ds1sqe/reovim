...
require_re '^- \[[xX]\] `dmesg --stats` / `cat /log/stats` report kernel log ring stats\.$' 'klog stats checkbox'
...
require_re '^- \[[xX]\] `help` / `help screentest` / `clear` / `screentest` prove shell help and erase-line mode diagnostics\.$' 'shell help and renderer checkbox'
...
require_re '^dropped_bytes=[0-9]+$' 'klog stats dropped-bytes line'
