...
missing_proof_shell_session_fact_evidence="$tmp_dir/missing-proof-shell-session-fact.md"
missing_proof_exec_parent_fact_evidence="$tmp_dir/missing-proof-exec-parent-fact.md"
...
write_passing_evidence() {
...

    {
        ...
        printf '  dmesg contains shell.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=<command>\n'
        printf '  dmesg contains exec.parent path=/bin/<command> ppid=2 parent_task=2\n'
        ...
        printf 'sched [status|tick|yield|sleep TICKS] - inspect scheduler state, tick, yield, or sleep current task\n'
        printf 'reovim-os> help proc\n'
        printf 'proc [processes|execs|address-spaces|page-tables|pages|memory-objects|media|pending|self|session|services|sources|tasks|waits|syscalls|continuations|scheduler] - inspect process state\n'
        ...
        printf 'reovim-os> help hello\n'
        printf 'hello - print a linked-bin syscall proof\n'
        printf 'reovim-os> hello\n'
        printf 'hello from linked bin\n'
        ...
        printf '  init - start userland session services\n'
        printf '  clear - clear framebuffer console and terminal\n'
        printf '  screentest - print renderer diagnostics\n'
        printf '    required rows: el: clean, el1: clean-left, el2: clean-all\n'
        printf '  pwd - print current kernel VFS directory\n'
        ...
        printf '  sched [status|tick|yield|sleep TICKS] - inspect scheduler state, tick, yield, or sleep current task\n'
        printf '  proc [processes|execs|address-spaces|page-tables|pages|memory-objects|media|pending|self|session|services|sources|tasks|waits|syscalls|continuations|scheduler] - inspect process state\n'
        printf '  exec PROGRAM [ARG...] - replace current process image\n'
        printf '  probe target - run lower hardware probe; try `probe help` or `cat /boot/probes`\n'
        printf '  launch [payload] - list or run registered payloads\n'
        printf '  reovim - run the default reovim payload alias\n'
        printf '  hello - print a linked-bin syscall proof\n'
        ...
        printf 'storage=none\n'
        printf 'last_sync_attempted=false\n'
        printf 'last_sync_reason=never-synced\n'
        printf 'syscall_records=42\n'
        printf 'syscall_continuation_records=0\n'
        ...
        printf 'storage=none\n'
        printf 'last_sync_attempted=false\n'
        printf 'last_sync_persistent=unavailable\n'
        printf 'last_sync_storage=none\n'
        printf 'last_sync_storage_capacity_bytes=0\n'
        printf 'last_sync_status=not-written\n'
        printf 'last_sync_bytes=0\n'
        printf 'last_sync_checksum=0\n'
        printf 'last_sync_verified=false\n'
        printf 'last_sync_reason=never-synced\n'
        ...
        printf -- '- pid=2 ppid=1 task=2 state=running path=/bin/sh exit=0 loader=linked-bin entry_fn=bin_sh\n'
        printf -- '- pid=73 ppid=2 task=73 state=running path=/bin/proc exit=0 loader=linked-bin entry_fn=bin_proc\n'
        ...
        printf 'shell: proof\n'
        printf 'shell.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=proof\n'
        printf 'exec.parent path=/bin/proof pid=3 ppid=2 task=3 parent_task=2\n'
        printf 'shell.status=ok\n'
        printf 'input.line_source=usb-keyboard usb_bytes=6 fallback_bytes=0 line_bytes=5\n'
        ...
        printf 'input.line_source=usb-keyboard usb_bytes=6 fallback_bytes=0 line_bytes=5\n'
        printf 'shell: proc\n'
        printf 'shell.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=proc\n'
        printf 'exec.parent path=/bin/proc pid=73 ppid=2 task=73 parent_task=2\n'
        printf 'shell.status=ok\n'
        printf 'input.line_source=usb-keyboard usb_bytes=6 fallback_bytes=0 line_bytes=5\n'
        printf 'shell: dmesg\n'
        ...
        printf 'shell.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=cat /log/dmesg\n'
        printf 'exec.parent path=/bin/cat pid=75 ppid=2 task=75 parent_task=2\n'
        printf '```\n\n'
        ...
        printf -- '- [x] `help device` / `help launch` / `help reovim` / `help hello` / `help halt` make boot inventory, payload, linked-bin, and shutdown `/bin` programs self-describing.\n'
        ...
        printf -- '- [x] `dump status` / `dump snapshot` / `dump sync` expose dump state, image identity, boot/device/proof/panic sections, retained last-sync state, and fail-closed persistence state.\n'
        printf -- '- [x] `halt` attempts dump sync before root daemon shutdown.\n'
        ...
        printf -- '- [x] `proc` reports `loader=linked-bin entry_fn=bin_proc` for the running `/bin/proc` process.\n'
        ...
        printf -- '- [x] `dmesg` contains `shell.session path=/bin/sh loader=linked-bin entry_fn=bin_sh line=<command>` audit lines for the typed commands.\n'
        printf -- '- [x] `dmesg` contains `exec.parent path=/bin/<command> ... ppid=2 ... parent_task=2` audit lines for shell-launched `/bin` programs.\n'
...
cp "$pass_evidence" "$missing_proof_shell_session_fact_evidence"
sed -i '/^  dmesg contains shell\.session path=\/bin\/sh loader=linked-bin entry_fn=bin_sh line=<command>$/d' "$missing_proof_shell_session_fact_evidence"
expect_failure "$missing_proof_shell_session_fact_evidence" "missing-proof-shell-session-fact"

cp "$pass_evidence" "$missing_proof_exec_parent_fact_evidence"
sed -i '/^  dmesg contains exec\.parent path=\/bin\/<command> ppid=2 parent_task=2$/d' "$missing_proof_exec_parent_fact_evidence"
expect_failure "$missing_proof_exec_parent_fact_evidence" "missing-proof-exec-parent-fact"
