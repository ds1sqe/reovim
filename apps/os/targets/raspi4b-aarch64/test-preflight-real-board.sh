...
expect_contains "$evidence" '`halt` attempts dump sync before root daemon shutdown.' "evidence halt dump-sync fact"
...
expect_contains "$evidence" '## Dump Analysis' "evidence dump analysis section"
expect_contains "$evidence" 'apps/os/targets/raspi4b-aarch64/analyze-dump.sh --expect-package reovim-os --expect-target aarch64-unknown-none --expect-profile shell-only --require-zero-drops <dump-file-or-mounted-bootfs>' "evidence dump analyzer command"
expect_contains "$evidence" 'apps/os/targets/raspi4b-aarch64/extract-dump-from-evidence.sh --output tmp/raspi4b-live-dump.txt --analyze <evidence-file>' "evidence dump extractor command"
expect_contains "$evidence" '`source=live-transcript`' "evidence live transcript source marker"
expect_contains "$evidence" '`persistent_sd_proof=false`' "evidence non-persistent dump marker"
expect_contains "$evidence" 'post-poweroff SD-card persistence proof' "evidence post-poweroff distinction"
