...
expect_contains "$preflight_output" "media_prepare_command=apps/os/targets/raspi4b-aarch64/prepare-boot-media.sh --bootfs $tmp_bootfs --evidence <final-evidence-file>" "media preparation command"
expect_contains "$preflight_output" "next=run prepare-boot-media without --skip-qemu before physical proof" "preflight next action"
...
expect_contains "$evidence" "- Media preparation command: apps/os/targets/raspi4b-aarch64/prepare-boot-media.sh --bootfs $tmp_bootfs --evidence <final-evidence-file>" "evidence media preparation command"
