...
expect_contains "$evidence" "- Media preparation command: apps/os/targets/raspi4b-aarch64/prepare-boot-media.sh --bootfs $tmp_bootfs --evidence $tmp_evidence --skip-qemu" "evidence actual media preparation command"
expect_not_contains "$evidence" "<final-evidence-file>" "evidence placeholder media preparation path"
...
expect_contains "$no_backup_evidence" "- Media preparation command: apps/os/targets/raspi4b-aarch64/prepare-boot-media.sh --bootfs $tmp_bootfs_no_backup --evidence $tmp_evidence_no_backup --skip-qemu --no-backup" "no-backup evidence actual media preparation command"
expect_not_contains "$no_backup_evidence" "<final-evidence-file>" "no-backup evidence placeholder media preparation path"
...
expect_contains "$auto_evidence" "- Media preparation command: apps/os/targets/raspi4b-aarch64/prepare-boot-media.sh --bootfs $tmp_bootfs_auto --evidence $tmp_evidence_auto --skip-qemu" "auto evidence actual media preparation command"
expect_not_contains "$auto_evidence" "<final-evidence-file>" "auto evidence placeholder media preparation path"
...
expect_contains "$evidence" "help read" "evidence targeted read help command"
...
expect_contains "$evidence" "help dump" "evidence targeted dump help command"
expect_contains "$evidence" "help sched" "evidence targeted sched help command"
expect_contains "$evidence" "help proc" "evidence targeted proc help command"
...
expect_contains "$evidence" "help hello" "evidence targeted hello help command"
expect_contains "$evidence" "hello" "evidence linked hello command"
...
expect_contains "$evidence" "\`help device\` / \`help launch\` / \`help reovim\` / \`help hello\` / \`help halt\` make boot inventory, payload, linked-bin, and shutdown \`/bin\` programs self-describing." "evidence remaining program help fact"
...
expect_contains "$evidence" "\`help pwd\` / \`help ls\` / \`help cd\` / \`help cat\` / \`help read\` / \`help mount\` make VFS navigation and TTY line reads self-describing." "evidence VFS/read targeted help fact"
...
expect_contains "$evidence" "dump status" "evidence dump status command"
expect_contains "$evidence" "dump snapshot" "evidence dump snapshot command"
expect_contains "$evidence" "dump sync" "evidence dump sync command"
expect_contains "$evidence" "cat /log/stats" "evidence VFS log stats command"
expect_contains "$evidence" "cat /log/events" "evidence VFS log events command"
...
expect_contains "$evidence" "\`dmesg\` pairs \`input.line_source=usb-keyboard\` with \`fallback_bytes=0\` immediately before typed command audit lines." "evidence USB line-source dmesg fact"
expect_contains "$evidence" "\`manual_next=type-shell-command\`" "evidence manual next fact"
expect_contains "$evidence" "\`probe usb-keyboard\` reports \`manual_next=type-shell-command\`." "evidence probe manual next fact"
...
expect_contains "$evidence" "\`dump status\` / \`dump snapshot\` / \`dump sync\` expose dump state, image identity, boot/device/proof/panic sections, retained last-sync state, and fail-closed persistence state." "evidence dump fail-closed fact"
expect_contains "$evidence" "\`cat /log/events\` reports structured kernel events with process/task identity." "evidence log events fact"
expect_contains "$evidence" "\`proc\` reports the live process table through a \`/bin\` process-management program." "evidence proc fact"
expect_contains "$evidence" "\`proc\` reports \`loader=linked-bin entry_fn=bin_proc\` for the running \`/bin/proc\` process." "evidence proc loader fact"
...
expect_contains "$evidence" "\`dmesg\` contains the retained \`probe usb-keyboard\` output with \`manual_next=type-shell-command\`." "evidence retained probe output fact"
