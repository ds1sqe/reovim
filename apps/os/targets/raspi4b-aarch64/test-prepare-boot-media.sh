...
expect_contains "$evidence" "\`help device\` / \`help launch\` / \`help reovim\` / \`help halt\` make boot inventory, payload, and shutdown \`/bin\` programs self-describing." "evidence remaining program help fact"
...
expect_contains "$evidence" "\`dump status\` / \`dump snapshot\` / \`dump sync\` expose dump state, image identity, boot/device/proof/panic sections, and fail-closed persistence state." "evidence dump fail-closed fact"
...
expect_contains "$evidence" "\`proc\` reports \`loader=source-image entry_fn=bin_proc\` for the running \`/bin/proc\` process." "evidence proc loader fact"
