...
async fn run(cli: Cli) -> std::io::Result<()> {
    match cli.command {
        Some(Commands::Server {
            ...
            tracing::info!("Starting reovim server (new architecture)");
            let server = Server::new(config);
            server.run().await
        }
        None => {
            // Default: start server with TCP fallback
            tracing::info!("Starting reovim server with default configuration");
